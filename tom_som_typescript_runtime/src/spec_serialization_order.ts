 ...
 * (and `spec_serialization_order.py` / the JavaScript
 * `spec_serialization_order.js`).
 *
...
export class SpecSerializationOrder {
  private _refl: SpecReflection;
  ...
  orderKey(path: string): number[] {
    ...
    for (let i = 1; i < segs.length; i++) {
      ...
      if (field !== null) {
        key.push(
          field.serializationOrder !== null
            ? field.serializationOrder
            : _UNORDERED_FALLBACK,
        );
        if (
   ...
   */
  orderPaths(paths: Iterable<string>): string[] {
    ...
    items.sort((a, b) =>
      _compareKeys(keys.get(a)!, a, keys.get(b)!, b),
    );
    return items;
  }
   ...
   */
  orderFormFields(formPath: string, fieldNames: Iterable<string>): string[] {
