...
class SomRustEmitter {
  ...
  String generateLibrary() {
    ...
    final buffer = StringBuffer()
      ..writeln(
        '// GENERATED by tom_specs_clitool SomRustEmitter '
      ...
      ..writeln('//')
      ..writeln(
        '// This file is the crate root; the generator wires the runtime '
        'dependency through a',
      )
      ..writeln(
        '// relative `path` entry in `Cargo.toml`, so the emitted source '
        'carries no on-disk',
      )
      ..writeln(
        '// path of its own. The metadata tree + navigation surfaces '
  ...
  void _allocateNames(
    ...
    for (final n in sortedClasses) {
      ...
      for (final f in cls.fields) {
        if (f.kind == SpecFieldKind.form) {
          _formNameFor['$n::${f.name}'] = _allocType(
            '$n${_pascal(f.name)}Form',
          );
        }
      }
    }
    ...
    for (final n in sortedClasses) {
      if (roots.contains(n)) {
        _modelVersionConst[n] = _allocValue(
          '${_screamingSnake(n)}_MODEL_VERSION',
        );
      }
    }
  }
  ...
  String _emitClass(
    ...
    if (isRoot) {
      ...
      final mvConst =
          _modelVersionConst[cls.name] ??
          '${_screamingSnake(cls.name)}_MODEL_VERSION';
      b
        ..writeln(
        ...
        ..writeln(
          "\t/// document's authoring `document_version` is editable "
          '(SOM §4.2). A non-editable',
        )
        ..writeln('\t/// stamp yields a `som::SomVersionError`.')
        ..writeln(
          '\tpub fn new(doc: som::DocRef, document_version: &str) '
          '-> Result<${cls.name}, som::SomVersionError> {',
        )
        ..writeln(
          '\t\tsom::check_som_model_version($mvConst, '
          'document_version)?;',
        )
        ..writeln(
          '\t\tOk(${cls.name} { node: som::SomNode::new(doc, '
        ...
        ..writeln(
          '\t/// editable by this object model **without erroring** '
          '(SOM §21) — the',
        )
        ..writeln(
          "\t/// non-erroring companion to the constructor's SOM §4.2 check, "
          'so a read-only',
        )
        ..writeln(
          '\t/// viewer can branch instead of handling a '
          '`som::SomVersionError`.',
        )
        ..writeln(
          '\tpub fn editability_for(document_version: &str) '
          '-> som::SomEditability {',
        )
        ...
        ..writeln()
        ..writeln(
          '\t/// Loads a `*.docspecs.yaml` document and returns the '
          'typed root with the',
        )
        ..writeln(
        ...
        ..writeln('\t\tlet tree = meta::${_snake(cls.name)}_meta_tree();')
        ..writeln(
          '\t\tlet doc = som::SpecDocument::from_file(path, &tree)'
          '.map_err(SomLoadError::Yaml)?;',
        )
        ..writeln('\t\tlet version = doc.model_version.clone();')
        ..writeln(
      ...
      b
        ...
        ..writeln(
          '\tpub fn new(doc: som::DocRef, path: String) '
          '-> ${cls.name} {',
        )
  ...
  void _emitCanHaveContent(StringBuffer b, bool value) {
    b
      ..writeln()
      ..writeln(
        '\t/// Whether this section **type** declares the standard '
        '`content` text leaf',
      )
      ..writeln(
        '\t/// (SOM §21) — a **structural** predicate answering "can '
        'this section hold',
      )
      ..writeln(
        '\t/// body text?" as a compile-time constant, without probing '
        'the document.',
      )
    ...
    switch (f.kind) {
      ...
      case SpecFieldKind.enumValue:
        if (f.enumType == null) {
          ...
          final parse =
              _parseName[f.enumType!] ?? 'parse_${_snake(f.enumType!)}';
          _writeComment(b, f.doc);
          b
            ..writeln('\tpub fn $acc(&self) -> String {')
            ..writeln(
              '\t\t$parse(&self.node.doc().borrow().content_or'
              '(&${_childPath(seg)}))',
            )
            ..writeln('\t}')
            ..writeln()
            ...
            ..writeln(
              '\t\tself.node.doc().borrow_mut().set_content'
              '(&path, value);',
            )
            ..writeln('\t}');
        }
      ...
      case SpecFieldKind.form:
        final formName =
            _formNameFor['$owner::${f.name}'] ?? '$owner${_pascal(f.name)}Form';
    ...
    switch (_scalarType(ff.type)) {
      case 'int':
        b
          ...
          ..writeln(
            '\t\tlet v = self.node.doc().borrow().form_field_or'
          ...
          ..writeln('\t\tlet path = self.node.path().to_string();')
          ..writeln(
            '\t\tlet text = match value { Some(v) => v.to_string(), '
            'None => String::new() };',
          )
          ..writeln(
      ...
      case 'bool':
        b
          ...
          ..writeln('\t\tlet path = self.node.path().to_string();')
          ..writeln(
            '\t\tlet text = match value { Some(true) => '
      ...
      default:
        b
          ..writeln('\tpub fn $acc(&self) -> String {')
          ..writeln(
            '\t\tself.node.doc().borrow().form_field_or'
            '(self.node.path(), $field)',
          )
          ..writeln('\t}')
          ..writeln()
  ...
  String _emitLoadError() {
    final b = StringBuffer()
      ..writeln(
        '/// Error surfaced by the generated one-call loaders '
        '(`load_yaml` / `load_file`):',
      )
      ..writeln(
        '/// either the metadata-driven decode failed '
        '(`som::SpecYamlError`) or the',
      )
      ..writeln(
        "/// document's authoring stamp is not editable by this object "
        'model',
      )
