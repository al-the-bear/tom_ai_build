...
''';
  ...
  // YRD6: title/id role fields are pure views onto the YRD3 headline/id
  // stores. Their values emit exactly once — as the item heading text / id
  // comment — never as form lines; a form line duplicating one is rejected.
  group('YRD6 — title/id role form fields', () {
    ...
    SpecModel model() => SpecModel.fromJson(cardsJson());
    String export(SpecDocument d) {
      final m = model();
      return SpecDocumentMarkdown(m, d).exportRoot(m.roots.first);
    }
    ...
    test('role values emit once — heading + id comment, no form line', () {
      final md = export(populated());
      expect(md, contains('<!--[CARD-ALPHA]--> Alpha Card'));
      expect(md, contains('Note: first card'));
      expect(md, isNot(contains('Card ID:')));
      expect(md, isNot(contains('cardId:')));
      expect(md, isNot(contains('Name:')));
    });

    test('a form line duplicating a role field is rejected, siblings kept', () {
      final md = '# <!--[C00]--> Card Doc\n\n'
          '## <!--[CARD-LST]--> Cards\n\n'
      ...
      expect(
        report.rejections.any(
            (r) => r.reason == SpecMarkdownRejectReason.roleFieldFormLine),
        isTrue,
        reason: report.rejections.join('\n'),
      );
      // The rejected role line and its continuation never reach the stores.
      final forms = report.forms['C00/CARD-LST-1/content'];
      expect(forms, {'note': 'kept note'});
      // The heading still restores the YRD3 views.
      expect(report.lists['C00/CARD-LST']?['ids'],
          {'C00/CARD-LST-1': 'CARD-ALPHA'});
    });
    ...
    test('round-trip restores the role-field views from the stores', () {
      ...
      expect(target.itemSectionId('C00/CARD-LST-1'), 'CARD-ALPHA');
      expect(target.headline('C00/CARD-LST-1'), 'Alpha Card');
      expect(target.formField('C00/CARD-LST-1/content', 'note'), 'first card');
      expect(export(target), md);
    });
  });
}
