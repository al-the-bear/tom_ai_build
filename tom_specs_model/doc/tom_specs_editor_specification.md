...
- **`tom_som_dart_v0`** (pkg `tom_som_dart_v0`, `publish_to: none`, generated `lib/tom_som_dart_v0.dart`) is the **typed facade for model major 0**: **3983** classes, each `extends SomNode`, mirroring the 14 roots (`D00SolutionBlueprint` … `D13CodeSpecsProjection`). That is **not** one class per model class: it is one per model class the documents reach (the 1252 in the meta, less the container root `DocSpecsProject`, which is a tree root rather than a document) **plus one per form section** — 2732 of those, since a form's fields are addressed through a class of their own. Typed access reads/writes through the runtime, so it stays a thin, validated projection of the underlying document tree rather than a second copy of state. See `tom_som_dart_v0/example/a_typed_access.dart` (`D00SolutionBlueprint(doc)` over a live document).
