# JavaScript tutorial — `tom_som_javascript_v0`

Using the TomSpecs Specification Object Model from JavaScript, end to end:
...
same plane is
[`tom_som_javascript_runtime/doc/generic_access.md`](../../tom_som_javascript_runtime/doc/generic_access.md).
...
`tom_som_javascript_v0` is the **typed** access path: one generated type per document
...
`tom_som_javascript_runtime` actually holds — the runtime carries the sparse,
...
together — `tom_som_javascript_v0` and `tom_som_javascript_runtime` always carry the
...
const rt = path.resolve('.', require('./package.json').tomSom.runtimePath);
const m = require('./tom_som_javascript_v0.js');
const { SpecDocument, SpecModel, validateDocument, yamlEncode, yamlDecode } =
  require(rt);
...
const y = yamlEncode(doc, m.d00SolutionBlueprintMetaTree, '1.0');
const decoded = yamlDecode(y, m.d00SolutionBlueprintMetaTree);
console.log(decoded.document.content('SBP/content'));
console.log(decoded.modelVersion);
```
...
./tool/regenerate_api_references.sh javascript_v0
```
...
Failures `throw`. `validateDocument` returns an array instead; an empty array is the pass.
...
- **Prefer the typed path.** Reach for the generic store only when the path is
  ...
  [`generic_access.md`](../../tom_som_javascript_runtime/doc/generic_access.md).
