 ...
 * subset the {@link SpecDocumentYaml} codec emits — the JavaScript runtime ships
 * with no external libraries (no js-yaml), so this hand-rolled indentation parser
 * stands in for `package:yaml` / PyYAML. It is a faithful port of the Java
 * runtime's hand-rolled `Yaml.java`.
 *
...
function _parseScalarKey(keyPart) {
...
class _Yaml {
  ...
  parseNestedAfterKey(parentIndent) {
    const i = this._nextSignificant(this.idx);
    if (i === -1) {
      return _newMap();
    }
   ...
   */
  parseBlockScalar(header, keyIndent) {
    let p = 1; // skip the leading '|' (or '>').
    let explicitIndent = -1;
    const digitsStart = p;
    while (p < header.length && header.charCodeAt(p) >= 48 && header.charCodeAt(p) <= 57) {
      p++;
    }
    if (p > digitsStart) {
