/// Emits the generated **metadata module** of the Java `v0` facade (the
...
/// and the **ID-tree** (section ids). Together they are the facade's **only**
...
class SomJavaMetaEmitter {
  ...
  /// accessor types. Must be the same model `SomJavaEmitter` saw — the two
  /// emitted files are one Java package and reference each other by name.
  final SpecModel model;

  /// The `v0`/`v1` project label. It only spells the package name in the
  /// generated banner and imports; the model version the facade reports comes
  /// from the model's own stamp, not from here.
  ...
  /// and not the other leaves the pair uncompilable.
  SomJavaMetaEmitter(
    this.model, {
  ...
  String generateLibrary() {
    ...
    final b = StringBuffer()
      ..writeln(
        '// GENERATED by tom_specs_clitool SomJavaMetaEmitter '
        '($versionLabel) — do not edit by hand.',
      )
      ..writeln('//')
      ..writeln('// The populated SOM metadata trees (SOM §7.2) and the two')
      ..writeln(
        '// generated access surfaces of SOM §8: the dot-notation '
        'tree',
      )
      ...
      ..writeln()
      ..writeln(
        '/** The generated SOM metadata module: populated metadata '
    ...
    b
      ..writeln(
        '$_i1// ── dot-notation accessor classes + '
    ...
    b
      ..writeln(
        '$_i1// ── ID-tree accessor classes (SOM §8) '
        '────────────────────────────────────',
      )
      ..writeln();
    ...
    b
      ..writeln(
        '$_i1// ── document-root metadata trees + access surface '
        'roots ──────────────────',
      )
      ..writeln();
  ...
  void _guardIdentifierCollisions(
    ...
    final clashes = [...typeClashes, ...fieldClashes, ...methodClashes]..sort();
    if (clashes.isNotEmpty) {
      throw StateError(
        'generated meta-module identifiers collide within '
  ...
  void _emitHelpers(StringBuffer b) {
    b
      ..writeln(
        "$_i1// metaCx builds a complex/section (or list-element) "
        "node with the bridge's",
      )
      ..writeln(
        '$_i1// cycle rule: a class already on the descent stack '
        'becomes a terminal',
      )
      ...
      ..writeln(
        '${_i3}BiFunction<Boolean, List<SomMetaNode>, SomMetaNode> '
        'build) {',
      )
      ...
      ..writeln()
      ..writeln(
        "$_i1// metaArgs builds an extra annotation's insertion-"
        'ordered argument map from',
      )
      ..writeln('$_i1// alternating key/value pairs.')
      ..writeln(
        '${_i1}private static Map<String, Object> metaArgs('
        'Object... kv) {',
      )
  ...
  String _lit(Object? value) {
    ...
    throw StateError(
      'unsupported annotation argument literal: '
  ...
  void _emitMetaChildrenBuilder(StringBuffer b, SpecClass cls) {
    b
      ..writeln(
        '$_i2// The metadata children of `${cls.name}` (SOM §7.2), '
        'bridge-identical.',
      )
      ..writeln(
        '${_i2}static List<SomMetaNode> metaChildren(Set<String> s) '
        '{',
      )
  ...
  void _emitFieldNode(StringBuffer b, SpecClass owner, SpecField f) {
    ...
    final typeName = _str(f.type ?? f.elementType ?? f.enumType ?? 'String');
    ...
    if (f.kind == SpecFieldKind.form) {
      ...
      for (var i = 0; i < f.formFields.length; i++) {
        ...
        final head =
            'new SomFormFieldMeta(${_str(ff.name)}, ${_str(ff.type)}, '
        ...
        final enums =
            'java.util.List.of(${ff.enumValues.map(_str).join(', ')})';
    ...
    if (target != null) {
      ...
      b.writeln(
        '${_i3}out.add(metaCx(${_str(target.name)}, s, '
        '${target.name}Nav::metaChildren, (r, c) -> {',
      );
      b.writeln(
        '${_i4}SomMetaNode n = new SomMetaNode($className, '
    ...
    b
      ..writeln('$_i3{')
      ..writeln(
        '$_i2${_i2}SomMetaNode n = new SomMetaNode($className, '
        '${_kindConst(f.kind)}, $typeName);',
      );
    for (final line in assigns) {
    ...
    if (element != null) {
      b.writeln(
        '$_i2${_i2}n.elementNode = metaCx(${_str(element.name)}, s, '
  ...
  void _emitRoot(StringBuffer b, SpecRoot root) {
    ...
    b
      ..writeln(
        '$_i1// ${root.type}MetaTree is the populated metadata tree '
        'of the `${root.type}`',
      )
      ..writeln('$_i1// document root (SOM §7.2).')
      ..writeln(
        '${_i1}public static final SomMetaTree ${root.type}MetaTree '
        '= build${root.type}MetaTree();',
      )
      ..writeln()
      ..writeln(
        '${_i1}private static SomMetaTree build${root.type}MetaTree'
        '() {',
      )
      ..writeln(
        '${_i2}SomMetaNode n = new SomMetaNode(${_str(root.type)}, '
    ...
    b.writeln(
      '${_i2}n.document = new SomDocMeta(${_str(root.title)}, '
    ...
    b
      ...
      ..writeln()
      ..writeln(
        '$_i1// ${root.type}Meta is the dot-notation access root of '
        '`${root.type}` (SOM §8):',
      )
      ..writeln('$_i1// `${root.type}Meta.<member>()….path` / `.meta()`.')
      ..writeln(
        '${_i1}public static final ${root.type}Nav ${root.type}Meta '
        '=',
      )
      ..writeln(
        '${_i3}new ${root.type}Nav(${root.type}MetaTree, '
        '${_str(seg)});',
      )
      ..writeln()
      ..writeln(
        '$_i1// $idSymbol is the ID-tree access root of '
        '`${root.type}` (SOM §8):',
      )
      ..writeln('$_i1// `$idSymbol.<SECTION_ID>()….path` / `.meta()`.')
      ..writeln('${_i1}public static final ${root.type}Id $idSymbol =')
      ..writeln(
        '${_i3}new ${root.type}Id(${root.type}MetaTree, '
        '${_str(seg)});',
      )
      ..writeln();
  }
  ...
  void _emitRootRegistry(StringBuffer b) {
    b
      ..writeln(
        '$_i1// ── document-root registry (SOM §8) '
        '─────────────────────────────────────',
      )
      ..writeln()
      ..writeln(
        '$_i1/** One document root: its class name, the path segment '
        'its access roots are',
      )
      ..writeln(
        '$_i1 * bound at, the populated metadata tree (SOM §7.2) and '
        'the two SOM §8 access',
      )
      ..writeln(
        '$_i1 * roots (typed as the common base so the entries are '
        'uniform). */',
      )
      ...
      ..writeln()
      ..writeln(
        '$_i1// SOM_META_ROOTS holds every document root this module '
        'generates, keyed by',
      )
      ..writeln(
        '$_i1// root class name and in model order. Emitted from the '
        'same root list that',
      )
      ..writeln(
        '$_i1// produced the trees above, so iterating it is '
        'equivalent to reading the',
      )
      ..writeln(
        '$_i1// generator input — no consumer needs a hand-kept copy '
        'of the root set.',
      )
      ..writeln(
        '${_i1}public static final Map<String, SomMetaRootEntry> '
        'SOM_META_ROOTS =',
      )
      ..writeln('${_i3}buildSomMetaRoots();')
      ..writeln()
      ..writeln(
        '${_i1}private static Map<String, SomMetaRootEntry> '
        'buildSomMetaRoots() {',
      )
      ..writeln(
        '${_i2}Map<String, SomMetaRootEntry> m = new '
        'LinkedHashMap<>();',
      );
    for (final root in _roots) {
      final seg = _rootSegment(root);
  ...
  void _emitNavClass(StringBuffer b, SpecClass cls) {
    b
      ..writeln(
        '$_i1// ${cls.name}Nav holds the dot-notation accessors of '
        '`${cls.name}` (SOM §8).',
      )
      ..writeln(
        '$_i1// Every method is one navigable position: `.path` is '
        'the absolute document',
      )
      ..writeln(
        '$_i1// path, `.meta()` the metadata node. Past a recursive '
        're-entry `.path` chains',
      )
      ..writeln(
        '$_i1// remain valid document positions while `.meta()` '
        'throws (the metadata tree',
      )
      ..writeln('$_i1// ends there).')
      ..writeln(
        '${_i1}public static final class ${cls.name}Nav extends '
        'SomMetaRef {',
      )
      ..writeln(
        '${_i2}public ${cls.name}Nav(SomMetaTree tree, String path) '
        '{',
      )
  ...
  String _navAccessor(Set<String> used, SpecClass cls, SpecField f) {
    ...
    if (_reservedAccessorNames.contains(base)) {
      throw StateError(
        'model field ${cls.name}.${f.name} collides with a '
  ...
  void _emitIdClass(StringBuffer b, SpecClass cls) {
    b
      ..writeln(
        '$_i1// ${cls.name}Id holds the ID-tree accessors of '
        '`${cls.name}` (SOM §8): methods',
      )
      ..writeln(
        '$_i1// named by section id (`-` → `_`), hoisted through '
        'id-less members so every',
      )
      ..writeln(
        '$_i1// reachable id is one step. `.path` and `.meta()` '
        'agree with the dot-notation',
      )
      ..writeln('$_i1// surface.')
      ..writeln(
        '${_i1}public static final class ${cls.name}Id extends '
        'SomMetaRef {',
      )
      ..writeln(
        '${_i2}public ${cls.name}Id(SomMetaTree tree, String path) '
        '{',
      )
      ..writeln('${_i3}super(tree, path);')
      ..writeln('$_i2}');
    for (final child in _idChildren(cls)) {
      if (_reservedAccessorNames.contains(child.name)) {
        throw StateError(
          'section id ${child.name} of ${cls.name} collides '
...
class _TypedBody {
  /// The declared Java return type of the generated getter, picked from the
  /// member's field kind — a `<Class>Nav` accessor for a complex or section
  /// member, a `SomListMetaRef<…>` for a list, `SomMetaRef` for any leaf.
  final String type;

  /// The method body, one entry per emitted line, written against the
  /// enclosing accessor's `tree` and `path`. The emitter supplies the
  /// method-body indent; only deeper nesting is spelled inside a line.
  final List<String> body;
...
class _IdChild {
  ...
  /// when it starts with a digit and keyword-sanitised, then suffixed `_2`,
  /// `_3`, … when two hoisted ids would collide. Generation fails loudly if it
  /// lands on a reserved `SomMetaRef` or `Object` member.
  final String name;
