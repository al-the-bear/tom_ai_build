...
class SpecOpsGenerator {
  ...
  void _writeHeader(StringBuffer b) {
    b.writeln(
      '// GENERATED by tom_specs_clitool/bin/generate_som.dart — '
      'do not edit by hand.',
    );
    b.writeln('//');
    b.writeln(
      '// Registers the snapshot/serialization contract '
      '(SpecClassOps) for every',
    );
    b.writeln(
      '// TomSpecs model class + section leaf, so the reflection-free '
      'engine can',
    );
    b.writeln(
      '// snapshot, clone and serialize the model without dart:mirrors '
      '(OE-2).',
    );
    b.writeln('//');
    b.writeln(
    ...
    b.writeln();
    b.writeln(
      '/// Registers [SpecClassOps] for every model class and section '
      'leaf.',
    );
  ...
  bool _isChildNode(ModelField f) {
    ...
    // content leaves.
    if (f.isList) {
      return f.listElementIsComplex || f.listElementIsContentSection;
    }
  ...
  String? _yamlScalarField(ModelClass cls) {
    final strings = cls.fields
        .where((f) => f.isString && !_isChildNode(f))
        .toList();
