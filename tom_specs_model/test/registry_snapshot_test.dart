...
void main() {
  group(
    'registry-backed engine on the real model (OE-2)',
    () {
      test(
        'registerSpecOps registers ops for the whole model + section leaves',
        () {
      ...
      test('toYaml serializes a @Form leaf as its named field values', () {
        ...
        final project = DocSpecsProject();
        project.solutionBlueprint.documentControl.header.form = DocSpecsForm(
          values: {'documentId': 'SBP-DEMO', 'version': '1.0'},
        );
      ...
      test(
        'COW snapshot shares an unchanged tree but re-clones the edited path',
        () {
          final project = DocSpecsProject();
