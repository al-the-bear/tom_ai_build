...
/// member names) and the **ID-tree** (section ids). Together they are the
/// facade's **only** path-addressing surface — there is no flat per-root
/// constant holder, because a constant cannot express a dynamic path (typed
/// prefix plus runtime-computed tail).
///
...
const Set<String> _reservedAccessorNames = {'new', 'path', 'meta', 'item'};
...
class SomRustMetaEmitter {
  ...
  String generateLibrary() {
    ...
    final b = StringBuffer()
      ..writeln(
        '// GENERATED by tom_specs_clitool SomRustMetaEmitter '
      ...
      ..writeln('//')
      ..writeln(
        '// `SomMetaTree` is `Rc`-based (not `Sync`/`Send`), so the '
        'trees cannot live in',
      )
      ..writeln(
        '// statics: each `<root>_meta_tree()` builds its tree per '
        'call (mirroring how',
      )
      ..writeln(
        "// the runtime's own tests build trees), and the "
  ...
  void _guardModuleCollisions(List<String> classNames, List<String> idClasses) {
  ...
  void _emitHelpers(StringBuffer b) {
    b
      ..writeln(
        '// meta_cx builds a complex/section (or list-element) node '
        "with the bridge's",
      )
      ..writeln(
      ...
      ..writeln(
        '\tkids: fn(&mut HashSet<String>) -> '
        'Vec<Rc<som::SomMetaNode>>,',
      )
      ..writeln(
        '\tbuild: impl FnOnce(bool, Vec<Rc<som::SomMetaNode>>) -> '
        'som::SomMetaNode,',
      )
  ...
  void _emitMetaChildrenBuilder(StringBuffer b, SpecClass cls) {
    ...
    b
      ..writeln(
        'fn meta_children_${_snake(cls.name)}($param: &mut '
  ...
  void _emitFieldNode(StringBuffer b, SpecClass owner, SpecField f) {
    ...
    add(
      'type_name',
      _strLit(f.type ?? f.elementType ?? f.enumType ?? 'String'),
    );
    ...
    if (f.kind == SpecFieldKind.form) {
      ...
      for (var i = 0; i < f.formFields.length; i++) {
        ...
        fields.add(
          'som::SomFormFieldMeta { name: ${_strLit(ff.name)}, '
          ...
          'refers_to: vec![${ff.refersTo.map(_strLit).join(', ')}] }',
        );
      }
      add(
        'form',
    ...
    if (element != null) {
      final elemArgs = <String>[
        ...
        if (element.headline != null) 'headline: ${_strLit(element.headline!)}',
      ...
      b
        ..writeln('\t\t{')
        ..writeln(
          '\t\t\tlet mut n = som::SomMetaNode { ${args.join(', ')}, '
          '..som::SomMetaNode::default() };',
        )
        ..writeln(
          '\t\t\tn.element_node = Some(meta_cx(${_str(element.name)}, '
    ...
    b.writeln(
      '\t\tRc::new(som::SomMetaNode { ${args.join(', ')}, '
      '..som::SomMetaNode::default() }),',
    );
  }
  ...
  void _emitRoot(StringBuffer b, SpecRoot root) {
    ...
    args.add(
      'document: Some(som::SomDocMeta { name: ${_strLit(root.title)}, '
    ...
    b
      ...
      ..writeln(
        '/// built per call — `SomMetaTree` is `Rc`-based and not '
        '`Sync`, so it cannot be',
      )
      ..writeln(
        '/// a `static`: construct once, keep the instance, and bind '
        'the access surfaces',
      )
      ..writeln(
        '/// below to it. The generated data is correct by '
        'construction, so a wiring',
      )
      ..writeln(
        '/// failure marks an emitter bug and panics instead of '
    ...
    if (cls != null) {
      b
        ...
        ..writeln(
          '\tlet children = meta_children_${_snake(root.type)}'
          '(&mut stack);',
        );
    }
    b
      ..writeln(
        '\tlet root = som::SomMetaNode { ${args.join(', ')}, '
        '..som::SomMetaNode::default() };',
      )
      ..writeln('\tsom::SomMetaTree::new(Rc::new(root))')
      ..writeln(
        '\t\t.expect("generated metadata tree failed to wire '
        '(emitter bug)")',
      )
      ..writeln('}')
      ..writeln()
      ..writeln(
        '/// The dot-notation access root of `${root.type}` '
        '(SOM §8):',
      )
      ..writeln(
        '/// `${snakeRoot}_meta(&tree).<member>()….path` / '
        '`.meta()`.',
      )
      ..writeln(
        'pub fn ${snakeRoot}_meta(tree: &som::SomMetaTree) -> '
        '${root.type}Nav<\'_> {',
      )
      ...
      ..writeln(
        'pub fn $idSymbol(tree: &som::SomMetaTree) -> '
        '${root.type}Id<\'_> {',
      )
  ...
  void _emitRootRegistry(StringBuffer b) {
    ...
    for (final root in _roots) {
      ...
      b
        ..writeln(
          '/// Erases `${snake}_meta`\'s accessor type to the common '
          '`SomMetaRef`.',
        )
        ..writeln(
          'fn ${snake}_nav_ref(tree: &som::SomMetaTree) -> '
          "som::SomMetaRef<'_> {",
        )
        ...
        ..writeln()
        ..writeln(
          '/// Erases `${_idName(seg)}`\'s accessor type to the common '
          '`SomMetaRef`.',
        )
        ..writeln(
          'fn ${snake}_id_ref(tree: &som::SomMetaTree) -> '
          "som::SomMetaRef<'_> {",
        )
    ...
    b
      ...
      ..writeln(
        '/// bound at, the constructor of its populated metadata tree '
        '(SOM §7.2), and',
      )
      ..writeln(
        '/// the two access roots (SOM §8) as constructors over that '
        'tree.',
      )
      ...
      ..writeln(
        '\t/// The path segment the root\'s access surfaces are bound '
        'at.',
      )
      ...
      ..writeln(
        "\tpub nav_ref: for<'a> fn(&'a som::SomMetaTree) -> "
        "som::SomMetaRef<'a>,",
      )
      ..writeln('\t/// Binds the ID-tree access root to a built tree.')
      ..writeln(
        "\tpub id_ref: for<'a> fn(&'a som::SomMetaTree) -> "
        "som::SomMetaRef<'a>,",
      )
      ..writeln('}')
      ..writeln()
      ..writeln(
        '/// Every document root this module generates, in model '
        'order. Emitted from the',
      )
      ..writeln(
        '/// same root list that produced the trees above, so '
  ...
  void _emitNavStruct(StringBuffer b, SpecClass cls) {
    b
      ...
      ..writeln(
        '/// Every method is one navigable position: `.path()` is '
      ...
      ..writeln(
        '/// chains remain valid document positions while `.meta()` '
        'returns an error',
      )
      ...
      ..writeln(
        '\tpub fn new(tree: &\'a som::SomMetaTree, path: String) -> '
        '${cls.name}Nav<\'a> {',
      )
      ..writeln(
        '\t\t${cls.name}Nav { meta_ref: som::SomMetaRef::new(tree, '
        'path) }',
      )
      ..writeln('\t}')
      ..writeln()
      ..writeln(
        '\t/// The absolute document path of this position '
        '(SOM §8 path grammar).',
      )
      ...
      ..writeln()
      ..writeln(
        '\t/// The metadata node at this position (an error past a '
        'recursive re-entry).',
      )
      ..writeln(
        '\tpub fn meta(&self) -> Result<Rc<som::SomMetaNode>, '
        'String> {',
      )
  ...
  _TypedBody _navGetter(SpecField f, String seg) {
    final rel =
        'self.meta_ref.tree, format!("{}/{}", self.meta_ref.path, '
        '"${_gseg(seg)}")';
    switch (f.kind) {
      case SpecFieldKind.complex:
      case SpecFieldKind.section:
        ...
        if (target != null) {
          return _TypedBody('${target.name}Nav<\'a>', [
            '${target.name}Nav::new($rel)',
          ]);
        }
  ...
  void _emitIdStruct(StringBuffer b, SpecClass cls) {
    b
      ...
      ..writeln(
        '/// methods named by section id (`-` → `_`), hoisted '
        'through id-less members',
      )
      ..writeln(
        '/// so every reachable id is one step. `.path` and '
        '`.meta()` agree with the',
      )
      ...
      ..writeln(
        '\t\t${cls.name}Id { meta_ref: som::SomMetaRef::new(tree, '
        'path) }',
      )
      ..writeln('\t}')
      ..writeln()
...
class _TypedBody {
  /// The generated `fn`'s return type, including the `'a` lifetime the Nav and
  /// Id structs are parameterised by — Rust needs it spelled out at every
  /// borrow, so the emitter carries it here rather than re-deriving it.
  final String type;

  /// The body, one entry per emitted line, written verbatim (tab-indented)
  /// inside the `fn`. A list rather than a single expression so an accessor
  /// that needs a statement before its result can grow one without changing
  /// this type or the emission loop.
  final List<String> body;
...
class _IdChild {
  /// The method name in the generated ID-tree type: the section id turned into
