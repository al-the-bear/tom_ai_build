/// The **Flutter-safe scripting facade** of the engine (`llm_and_d4rt_tools.md`
/// §3 / §5 / §8.1).
///
/// The TomSpecs editor links the engine **in-process** to give its agent a D4rt
/// scripting surface over the live document (`llm_and_d4rt_tools.md` §3.1). The
/// full barrel (`tom_spec_engine.dart`) also exports the RAG **memory** plane,
/// which depends on `tom_brain_memory` (sqlite3 FFI / `vec0` — a server-only,
/// non-Flutter surface, `llm_and_d4rt_tools.md` §2). The editor needs only the
/// **scripting** surface — the `spec` scope bound to a live [SpecController],
/// the scope registry, and the script tools — none of which touch memory.
///
...
///
/// Beyond the scripting surface it also re-exports the **memory-free
/// `llm_and_d4rt_tools.md` §8 tool classes** the editor's `AgentToolsModule`
/// registers as MCP tools (`llm_and_d4rt_tools.md` §8): [DocTools] (the
/// `llm_and_d4rt_tools.md` §6 `doc_*` search / reflect / add-node logic over a
/// [SpecController]) and [FileTools] (the audited `file_*` surface over a
/// [SpecFileFacade]). The `mem_*` `MemoryTools` are **not** re-exported here —
