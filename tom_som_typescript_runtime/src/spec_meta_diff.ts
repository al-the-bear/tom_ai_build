 ...
 * of `tom_som_dart_runtime/lib/src/spec_meta_diff.dart` (and the
 * Python/Java/JavaScript ports).
 *
 ...
 * {@link buildSomMetaTree} derives the same tree from the exported meta-JSON
 * at runtime — the two must be field-for-field identical for every node.
 * Tests compare them with this function, which returns a human-readable
 * description of the **first** difference found (with the node's position),
 * or `null` when the subtrees agree completely.
 */
 ...
 */
export function somMetaNodeDiff(
  a: SomMetaNode,
  b: SomMetaNode,
  at: string = '<root>',
): string | null {
  function diff(field: string, va: unknown, vb: unknown): string | null {
  ...
  if (aElem) {
    return somMetaNodeDiff(
      a.elementNode as SomMetaNode,
      b.elementNode as SomMetaNode,
      `${at}/§element`,
    );
  }
  return null;
...
function _valueEq(a: unknown, b: unknown): boolean {
...
function _listEq(a: unknown[], b: unknown[]): boolean {
  if (a.length !== b.length) {
    return false;
  }
  for (let i = 0; i < a.length; i++) {
