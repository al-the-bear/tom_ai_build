...
### 10.3 Accessors for the shipped data files
...
**Three artifacts, one argument.** The model is the largest but not the only
thing a generation run ships as data, and each of the others was reached the
same way and failed the same way:

| Artifact | Shipped as | Dart accessor |
|---|---|---|
| the object model | `meta/spec_model.meta.json` | `somSpecModel` in `lib/<package>_model.dart` |
| the 14 DocSpecs schemas | `schemas/<id>/<id>.<major>.0.docspecs-schema.yaml` | `somDocSpecsSchema(id)` in `lib/<package>_schemas.dart` |
| the CodeSpecs area catalogue | `tom_specs_model`'s `generated-doc/codespecs/codespecs_areas.json` | `somCodeSpecsAreas` in `tom_specs_model`'s `lib/codespecs_areas.dart` |

The rest of this section is written about the model because that is where the
argument was first made and measured; **it applies unchanged to the other two**,
which are generated by the same runs, embedded from the same objects, and kept
out of their barrels for the same reason. Sizes differ by two orders of
magnitude — 6.42 MB, 3.7 MB and 16 kB — and the smallest of the three has no
opt-out argument at all; it is still its own library, because a catalogue of
CodeSpecs areas is not part of the object model.

**One library for fourteen schemas, not fourteen libraries.** A library per
document root would let a consumer pay for the roots it opens, and the sample
evidence says consumers open one or three at a time. It is one library because
Dart cannot tree-shake entries out of a `const Map` either way, so the choice is
between fourteen imports a consumer must know about and one that costs 3.7 MB
when opted into — and splitting it later is a change to one emitter rather than
one consumers have to follow.
...
**The schema and catalogue accessors are Dart-only, and the asymmetry is the
same one.** The other eight planes ship the identical `schemas/` tree and read
the identical `codespecs_areas.json`; none has an accessor over either. That is
not an oversight carried forward but the same open question stated twice more:
whether a Go module, a wheel, a jar or a crate can locate its own shipped data
is a fact about that packaging, and answering it for eight planes and three
artifacts is a port with a design decision in it rather than a mechanical one.
Python's model accessor already shows the two answers differ — it reads the file
and Dart embeds it — so the ports are owed the question, not the copy.
