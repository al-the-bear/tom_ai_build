...
| **CE-CC** ClientConfiguration | Client-app + machine scope (no user); single-moded per §11. | **Built on:** the `TomProperty<T>` family (`tom_core_flutter`, `tomclient/resources/tom_properties.dart`) + `TomConfigResourceProvider` (`tom_core_kernel`).<br>**Annotations:** `@CsClientConfig` on the holder class; per-field constraint attributes.<br>**Example:** `@CsClientConfig class AppClientConfig { final serverUrl = TomProperty<String>(…); }` | **Candidate gap** — a dedicated **client-settings holder** class (aggregating `TomProperty` fields with load/persist) is not present; if confirmed, the concrete holder lands in `tom_core_codespecs`. | Same per-field constraint attributes as CE-CF (defaults, formats, ranges). |
...
| **CE-WF** Workflow *(deferred, §4.3)* | Long-running multi-step business process; SOM home DEPRWO (D02). | Mapping-only — reserved kind value; no surfaces. Would require a state-machine / process runtime. | **Full substrate missing** — no core state-machine or process-instance persistence exists; the largest-distance deferred part; no promotion roadmap todo yet. | Future annotation family undefined until a substrate exists. |
| **CE-NT** Notification *(deferred, §4.3)* | Outbound user notifications, email first; SOM home NM (D06, ATS). | Mapping-only — reserved kind value; no surfaces. | **Transport substrate gap (`csex10`)** — an abstract **notification-transport** class (not hard-coding SMTP) + an SMTP implementation; credentials as **secret-marked CE-CF** config. Promotion is gated on this. | Future `@CsNotification`; message templates keyed via CE-TX (`CsMessageKey`). |
| **CE-LG** AuditLog *(deferred, §4.3)* | Business-relevant audit trail — **distinct from diagnostic logging**; SOM home SAS (D08). | Mapping-only — reserved kind value; no surfaces. Kernel logging is diagnostic, not audit. | **Audit-hook gap (`csex9`)** — hooks at the two chokepoints: the CE-API pipeline (`TomEndpointHandler`) and the CE-DB write path. Promotion runs via `csra7` once the hooks exist. | Future `@CsAudited` modifier on `@CsEndpoint` / repository writes. |
...
- **Deferred:** CE-RP (`csex5`/`csex11`), CE-NT (`csex10`), CE-LG (`csex9`),
  CE-WF (no substrate, no roadmap todo).
...
| **CE-LG** | AuditLog | `auditLog` | `AuditAndLogging` (`13234`) + `SecurityEventLoggingPolicy` + `AuditLogFormat` — `security_and_access_model.dart` (D08 SAS) | Logging & audit trail — who did what, when. **Must be in the spec**, woven into the functionality descriptions rather than declared standalone (it is not doable purely declaratively); `tom_core` needs the correct **audit-logging hooks**, and some aspects overlap with CE-RP reporting. |
...
| CE-NT (§4.3, deferred) | Email delivery — the definition names the channel abstractly; the transport is framework roadmap / future CE-NT. |
...
- **Email transport** — no SMTP/mailer substrate in `tom_core`; until it
  lands, `email` remains a named-but-unwired channel.
