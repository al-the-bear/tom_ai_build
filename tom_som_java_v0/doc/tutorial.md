# Java tutorial — `tom_som_java_v0`

Using the TomSpecs Specification Object Model from Java, end to end:
...
same plane is
[`tom_som_java_runtime/doc/generic_access.md`](../../tom_som_java_runtime/doc/generic_access.md).
...
`tom_som_java_v0` is the **typed** access path: one generated type per document
...
`tom_som_java_runtime` actually holds — the runtime carries the sparse,
...
```bash
add the Maven dependency (group `io.github.al-the-bear`)
```

Both halves are versioned to the TomSpecs **model version** and must move
together — `tom_som_java_v0` and `tom_som_java_runtime` always carry the
...
public class Tutorial {
  public static void main(String[] args) throws Exception {
    ...
    TomSomV0.D00SolutionBlueprint bp = new TomSomV0.D00SolutionBlueprint(doc);

    // 2 — edit through named members, not string paths.
    bp.content("A platform that unifies our fragmented order systems.");
    bp.currentLandscape()
    ...
    String y = SpecDocumentYaml.encode(
        doc, TomSomV0Meta.D00SolutionBlueprintMetaTree, "1.0");
    SpecYamlContents decoded =
        SpecDocumentYaml.decode(y, TomSomV0Meta.D00SolutionBlueprintMetaTree);
    System.out.println(decoded.document.content("SBP/content"));
    System.out.println(decoded.modelVersion);
  }
}
```
...
```bash
./build_jar.sh && java -cp build Tutorial
```
...
./tool/regenerate_api_references.sh java_v0
```

That renders it with `javadoc`. The reasoning behind not committing it, and
...
Failures throw. `SpecValidator.validateDocument` returns a list instead; an empty list is the pass.
...
- **Prefer the typed path.** Reach for the generic store only when the path is
  ...
  [`generic_access.md`](../../tom_som_java_runtime/doc/generic_access.md).
