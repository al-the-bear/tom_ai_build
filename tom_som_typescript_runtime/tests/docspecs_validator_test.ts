 ...
 * a port of `tom_som_javascript_runtime/tests/docspecs_validator_test.js`
 * (itself a port of the Python/Dart reference suite): the generic schema-free
 * parse, schema loading (with warnings for unsupported features), the
 * structured violation list, and the acceptance criterion — the
 * markdown-codec-emitted Solution Blueprint sample validates cleanly against the
 * generated `solution-blueprint` schema.
 *
 * Build with `tsc`, then run `node dist/tests/docspecs_validator_test.js`.
 * Exit code 0 == green.
 */
...
function _schema(): DocSpecsSchema {
  return DocSpecsSchema.fromYamlText(_SCHEMA_YAML);
}

function _validate(md: string): DocSpecsViolation[] {
  return new DocSpecsValidator(_schema()).validateMarkdown(md);
}

function _detail(violations: DocSpecsViolation[]): string {
  return violations.map((v) => String(v)).join('; ');
}

function _listEqual(a: readonly unknown[], b: readonly unknown[]): boolean {
  return a.length === b.length && a.every((x, i) => x === b[i]);
}

// --- DocSpecsDocument.parse (generic, schema-free) ----------------------------

function testParseTree(): void {
...
function testParseFenceShield(): void {
...
function testParseMalformedHeading(): void {
  const doc = DocSpecsDocument.parse(
    '# <!--[D00]--> Demo Document\n' + '\n' + '## Plain Heading\n',
  );
...
function testSchemaLoading(): void {
...
function testSchemaResolution(): void {
  const schema = _schema();
...
function testSchemaWarnings(): void {
...
function testValidateMissingRequiredSection(): void {
...
function testValidateIdPatternMismatch(): void {
...
function testValidateUnknownSection(): void {
...
function testValidateDisallowedPosition(): void {
...
function testValidateMissingRequiredField(): void {
...
function testValidateFieldPatternMismatch(): void {
...
function testValidateTextRequired(): void {
...
function testValidateTooManyItems(): void {
  const md =
    `${_VALID_DOC}\n` +
    '### <!--[GSUM]--> Summary\n\nOne.\n\n' +
    '### <!--[GSUM]--> Summary\n\nTwo.\n';
...
function testValidateFormatMismatch(): void {
  const md = `${_VALID_DOC}\n## <!--[DIAG]--> Diagram\n\nno fence here\n`;
  const v = _validate(md);
...
function testValidateRootMismatch(): void {
...
function testValidateNeverFailFast(): void {
  const md = _VALID_DOC.replace('Author: Alice\n', '')
...
function testBindDocspecsMarkdown(): void {
...
function testDr7Acceptance(): void {
