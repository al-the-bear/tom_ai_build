...
`tom_som_cpp_v0` is the typed TomSpecs object-model facade for C++. It depends on `tom_som_cpp_runtime`. Both are versioned to the TomSpecs **model version** (currently `1.3.1`) — pin to that version so your document reads and writes match the model the facade was generated from.
...
```bash
make -C ../tom_som_cpp_runtime dist   # tom_som_cpp_runtime-1.3.1.tar.gz
make dist                             # tom_som_cpp_v0-1.3.1.tar.gz
```
...
Both `tom_som_cpp_runtime` and `tom_som_cpp_v0` carry version `1.3.1`, taken from the TomSpecs model version. When you upgrade the model, regenerate and move to the new matching version so the facade and your stored documents stay in step.
