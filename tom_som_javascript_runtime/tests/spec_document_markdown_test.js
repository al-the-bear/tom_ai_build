...
function testExportStoredItemId() {
  const doc = new SpecDocument();
  const item = doc.addListItem('D00/D00-ITM', 'D01-CUSTOM');
  doc.setContent(`${item}/D01-LBL`, 'Custom-id item');
  const md = _export(doc);
  // YRD3: a stored id IS the item's heading id in md — the positional
  // fallback is used only for anonymous items.
...
function testRoundTripByteStable() {
...
function testRoundTripRichMarkdown() {
  const doc = new SpecDocument();
...
function testRoundTripStoredItemId() {
  const doc = new SpecDocument();
  const item = doc.addListItem('D00/D00-ITM', 'D01-CUSTOM');
  doc.setContent(`${item}/D01-LBL`, 'Custom-id item');
  const md1 = _export(doc);
  // YRD3: a stored id round-trips through md — it is the heading id and is
  // recovered on parse.
...
function testRoundTripLabelShapedContinuation() {
  const doc = new SpecDocument();
  doc.setFormField(
    'D00/D00-HDR',
    'author',
...
function testFormPreambleParses() {
  const md =
    '# <!--[D00]--> Demo Document\n\n' +
    '## <!--[D00-HDR]--> Header\n\n' +
...
function testFormPreambleEmits() {
  const doc = new SpecDocument();
...
function testFormPreambleRoundTrip() {
  const doc = new SpecDocument();
...
function testFormPreambleLabelShaped() {
...
function testRejectMalformedHeading() {
  const md =
    '<!-- docspec: demo-document/1.0 -->\n' +
    '# <!--[D00]--> Demo Document\n\n' +
...
function testRejectOrphanPreamble() {
  const md =
    'stray preamble text\n' +
    '# <!--[D00]--> Demo Document\n\n' +
    '## <!--[D00-OVR]--> Overview\n\n' +
...
function testRejectKindMismatch() {
  const md =
    '# <!--[D00]--> Demo Document\n\n' +
    '## <!--[D00-OVR]--> Overview\n\n' +
    'kept\n\n' +
...
function testRejectMissingValue() {
  const md =
    '# <!--[D00]--> Demo Document\n\n' +
    '## <!--[D00-OVR]--> Overview\n\n' +
    '## <!--[D00-ST]--> Status\n\n' +
...
function testCaseInsensitiveLabels() {
  const md =
    '# <!--[D00]--> Demo Document\n\n' +
    '## <!--[D00-HDR]--> Header\n\n' +
...
function testFenceShieldedHeadingsStayBody() {
  const md =
    '# <!--[D00]--> Demo Document\n\n' +
    '## <!--[D00-OVR]--> Overview\n\n' +
    '```\n' +
