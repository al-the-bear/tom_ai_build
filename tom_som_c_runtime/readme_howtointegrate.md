# Integrating tom_som_c_runtime

`tom_som_c_runtime` is the generic, value-free TomSpecs object-model runtime for
C. It has **zero external dependencies** (C standard library only) and is
versioned to the TomSpecs **model version** — the same version the typed facade
`tom_som_c_v0` reports. Pin both to that version so your document reads and
writes match the model the facade was generated from.

Most projects depend on the typed facade `tom_som_c_v0` (which finds this
...
C has no package registry, so distribution is by **library + headers +
pkg-config file** (installed system-wide or vendored) or by **source tarball**.
...
cc myapp.c $(pkg-config --cflags --libs tom_som_c_runtime) -o myapp
```
...
The emitted `tom_som_c_runtime.pc` carries `Version = <model version>`, so
`pkg-config --modversion tom_som_c_runtime` reports the model version.
...
`libtom_som_c_runtime.a` (or `.so`) to your own build.
...
cc myapp.c -I../tom_som_c_runtime/include \
   ../tom_som_c_runtime/build/libtom_som_c_runtime.a -o myapp
```

This is exactly how `tom_som_c_v0` consumes the runtime — the generated facade
...
`tom_som_c_runtime` and `tom_som_c_v0` both carry the TomSpecs model version:
...
The runtime builds with any C11 compiler and has no external dependencies:

```sh
make                 # static + shared library + pkg-config file
make test            # conformance harness against the shared corpus
make unit            # standalone unit tests
