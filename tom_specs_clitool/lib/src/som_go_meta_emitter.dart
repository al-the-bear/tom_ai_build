/// Emits the generated **metadata module** of the Go `v0` facade (the Go
...
class SomGoMetaEmitter {
  ...
  String generateLibrary() {
    ...
    final b = StringBuffer()
      ..writeln(
        '// GENERATED by tom_specs_clitool SomGoMetaEmitter '
  ...
  void _guardPackageCollisions(
    ...
    if (clash.isNotEmpty) {
      throw StateError(
        'generated meta-module identifiers collide with '
        'facade identifiers in package $packageName: ${clash.join(', ')}',
      );
    }
    if (meta.length !=
        5 + classNames.length * 3 + idClasses.length * 2 + _roots.length * 3) {
      throw StateError(
  ...
  void _emitHelpers(StringBuffer b) {
    b
      ..writeln(
        '// metaCx builds a complex/section (or list-element) node '
        "with the bridge's",
      )
      ..writeln(
        '// cycle rule: a class already on the descent stack becomes '
        'a terminal',
      )
      ..writeln('// re-entry node (Recursive: true, no children).')
      ..writeln(
        'func metaCx(cls string, stack map[string]bool, kids '
      ...
      ..writeln()
      ..writeln(
        '// metaIntPtr widens an optional-int literal into the '
        "runtime's *int slots",
      )
      ...
      ..writeln()
      ..writeln(
        '// mustMetaTree wires a generated root node into its '
        'SomMetaTree. The generated',
      )
      ..writeln(
        '// data is correct by construction, so a wiring error marks '
        'an emitter bug and',
      )
  ...
  void _emitMetaChildrenBuilder(StringBuffer b, SpecClass cls) {
    b
      ..writeln(
        'func metaChildren${cls.name}(s map[string]bool) '
        '[]*som.SomMetaNode {',
      )
  ...
  void _emitFieldNode(StringBuffer b, SpecClass owner, SpecField f) {
    ...
    if (target != null) {
      ...
      b
        ..writeln(
          '\t\tmetaCx(${_str(target.name)}, s, '
          'metaChildren${target.name}, func(r bool, c []*som.SomMetaNode) '
          '*som.SomMetaNode {',
        )
        ..writeln(
          '\t\t\treturn &som.SomMetaNode{${args.join(', ')}, '
    ...
    if (element != null) {
      final elemArgs = <String>[
        ...
        if (element.headline != null) 'Headline: ${_str(element.headline!)}',
      ...
      b
        ...
        ..writeln(
          '\t\t\tn.ElementNode = metaCx(${_str(element.name)}, s, '
  ...
  void _emitRoot(StringBuffer b, SpecRoot root) {
    ...
    args.add(
      'Document: &som.SomDocMeta{Name: ${_str(root.title)}, '
    ...
    if (cls != null) {
      args.add(
        'Children: metaChildren${root.type}(map[string]bool{'
        '${_str(root.type)}: true})',
      );
    }
    ...
    b
      ..writeln(
        '// ${root.type}MetaTree is the populated metadata tree of '
        'the `${root.type}`',
      )
      ..writeln('// document root (SOM §7.2).')
      ..writeln(
        'var ${root.type}MetaTree = mustMetaTree(&som.SomMetaNode{'
        '${args.join(', ')}})',
      )
      ..writeln()
      ..writeln(
        '// ${root.type}Meta is the dot-notation access root of '
        '`${root.type}` (SOM §8):',
      )
      ..writeln('// `${root.type}Meta.<Member>()….Path` / `.Meta()`.')
      ..writeln(
        'var ${root.type}Meta = new${root.type}Nav('
        '${root.type}MetaTree, ${_str(seg)})',
      )
      ..writeln()
      ..writeln(
        '// $idSymbol is the ID-tree access root of `${root.type}` '
        '(SOM §8):',
      )
      ..writeln('// `$idSymbol.<SECTION_ID>()….Path` / `.Meta()`.')
      ..writeln(
        'var $idSymbol = new${root.type}ID(${root.type}MetaTree, '
        '${_str(seg)})',
      )
      ..writeln();
  }
  ...
  void _emitRootRegistry(StringBuffer b) {
    b
      ...
      ..writeln()
      ..writeln(
        '// SomMetaRootEntry is one document root of this model: its '
        'class name and',
      )
      ..writeln(
        '// section id, its populated metadata tree, and the two '
        'access roots',
      )
      ..writeln('// (SOM §8) bound to it, as the common som.SomMetaRef base.')
      ..writeln('type SomMetaRootEntry struct {')
      ..writeln(
        '\t// Type is the root class name, e.g. '
        '`D00SolutionBlueprint`.',
      )
      ..writeln('\tType string')
      ..writeln(
        '\t// Segment is the path segment the two access roots are '
        'bound at — the',
      )
      ..writeln(
        '\t// root section id where the model gives one, else the '
        'class name.',
      )
      ..writeln('\tSegment string')
      ..writeln(
        '\t// Tree is the populated metadata tree of this root '
        '(SOM §7.2).',
      )
      ..writeln('\tTree *som.SomMetaTree')
      ..writeln('\t// Nav is the dot-notation access root.')
      ..writeln('\tNav *som.SomMetaRef')
      ..writeln('\t// ID is the ID-tree access root.')
      ..writeln('\tID *som.SomMetaRef')
      ..writeln('}')
      ..writeln()
      ..writeln(
        '// SomMetaRoots is every document root this module '
        'generates, keyed by root',
      )
      ..writeln(
        '// class name. Emitted from the same root list that produced '
        'the trees and',
      )
      ..writeln(
        '// access roots above, so ranging over it is equivalent to '
        'reading the',
      )
      ..writeln(
        '// generator input — no consumer needs a hand-kept copy of '
        'the root set.',
      )
      ..writeln('var SomMetaRoots = map[string]SomMetaRootEntry{');
    for (final root in _roots) {
      final seg = _rootSegment(root);
      final idSymbol = _idName(seg);
      b.writeln(
        '\t${_str(root.type)}: {Type: ${_str(root.type)}, '
        'Segment: ${_str(seg)}, Tree: ${root.type}MetaTree, '
        'Nav: &${root.type}Meta.SomMetaRef, '
        'ID: &$idSymbol.SomMetaRef},',
      );
    }
    b
      ..writeln('}')
  ...
  void _emitNavStruct(StringBuffer b, SpecClass cls) {
    b
      ..writeln(
        '// ${cls.name}Nav holds the dot-notation accessors of '
        '`${cls.name}` (SOM §8).',
      )
      ..writeln(
        '// Every method is one navigable position: `.Path` is the '
        'absolute document',
      )
      ..writeln(
        '// path, `.Meta()` the metadata node. Past a recursive '
        're-entry `.Path` chains',
      )
      ..writeln(
        '// remain valid document positions while `.Meta()` returns '
        'an error (the',
      )
      ...
      ..writeln()
      ..writeln(
        '// new${cls.name}Nav binds a ${cls.name}Nav accessor to a '
        'tree and a path.',
      )
      ..writeln(
        'func new${cls.name}Nav(tree *som.SomMetaTree, path string) '
        '*${cls.name}Nav {',
      )
      ..writeln(
        '\treturn &${cls.name}Nav{SomMetaRef: som.SomMetaRef{Tree: '
        'tree, Path: path}}',
      )
      ..writeln('}');
    final used = <String>{};
  ...
  void _emitIdStruct(StringBuffer b, SpecClass cls) {
    b
      ..writeln(
        '// ${cls.name}ID holds the ID-tree accessors of '
        '`${cls.name}` (SOM §8): methods',
      )
      ..writeln(
        '// named by section id (`-` → `_`), hoisted through id-less '
        'members so every',
      )
      ..writeln(
        '// reachable id is one step. `.Path` and `.Meta()` agree '
      ...
      ..writeln()
      ..writeln(
        '// new${cls.name}ID binds a ${cls.name}ID accessor to a '
        'tree and a path.',
      )
      ..writeln(
        'func new${cls.name}ID(tree *som.SomMetaTree, path string) '
        '*${cls.name}ID {',
      )
      ..writeln(
        '\treturn &${cls.name}ID{SomMetaRef: som.SomMetaRef{Tree: '
        'tree, Path: path}}',
      )
      ..writeln('}');
    for (final child in _idChildren(cls)) {
