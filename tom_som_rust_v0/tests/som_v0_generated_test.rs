...
use tom_som_rust_v0::{ProjectDefinition, PROJECT_DEFINITION_MODEL_VERSION};
...
fn root_and_parity() {
    ...
    assert_eq!(pd.node.path(), "PD", "root segment");

    // Typed write → generic read.
    pd.set_content("A clear vision");
    assert_eq!(doc.borrow().content_or("PD/content"), "A clear vision");

    // Generic write → typed read.
    doc.borrow_mut().set_content("PD/content", "Revised vision");
    assert_eq!(pd.content(), "Revised vision");

    // Unset leaf reads as empty string.
    let fresh = ProjectDefinition::new(new_doc(), "").unwrap();
    ...
    assert_eq!(csa.node.path(), "PD/currentStateAnalysis");
    ...
    assert_eq!(doc.borrow().content_or("PD/currentStateAnalysis/probe"), "x");
}

#[test]
fn typed_list() {
    let doc = new_doc();
    let pd = ProjectDefinition::new(doc.clone(), "").unwrap();
    let metrics = pd.current_state_analysis().operational_metrics();
...
#[test]
fn model_version() {
    assert_eq!(PROJECT_DEFINITION_MODEL_VERSION, "0.0");
    let pd = ProjectDefinition::new(new_doc(), "").unwrap();
...
fn version_check() {
    ...
    assert!(ProjectDefinition::new(new_doc(), "").is_ok(), "empty stamp");
    assert!(ProjectDefinition::new(new_doc(), "0.0").is_ok(), "equal stamp");

    // Newer minor → rejected with a SomVersionError.
    assert!(
        ProjectDefinition::new(new_doc(), "0.1").is_err(),
        "newer-minor stamp must be rejected"
    );

    // Different major → rejected with a SomVersionError.
    assert!(
        ProjectDefinition::new(new_doc(), "1.0").is_err(),
