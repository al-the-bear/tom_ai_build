...
- **Deferred:** CE-RP (`csex5`/`csex11`), CE-NT (`csex10`),
  CE-WF (substrate survey/recommendation owned by `csex13`). **CE-LG is
  deferred-but-decidable:** its blocking hook gap is closed (`csex9` — the
  `tom_core_server` `audit` module), so the §4.3 promotion criterion is
  satisfied and `csra7` decides it on the merits rather than on availability.
