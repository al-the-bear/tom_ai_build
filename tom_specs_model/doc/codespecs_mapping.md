...
typed-reference family the constructors consume is authored, in
`tom_code_specs/lib/src/annotations/cross_part_refs.dart`.
The gap columns
...
| **CE-CL** Client | A client application of the system: which screens/forms/flows it comprises, its platform targets, its entry route. | **No `tom_core` basis by design** — the client descriptor is a **pure plain annotated Dart class** (coding form 2).<br>**Annotations:** `@CsClient` on the descriptor.<br>**Example:** `@CsClient(platforms: […]) class BackofficeClient { … }` referencing routes (`CsRouteRef`) and forms. | **Gap** — the **client-application descriptor** class (a simple concrete data class) lives in `tom_core_codespecs`. | `@CsClient` attributes: **platform targets**, **entry route**, included flows — none expressible in plain code. |
...
|---|-------|-----------------|------|--------------|
| **1** | Shared const catalogues | CE-ER, CE-TX; CE-AZ catalogues | **E** | `csrb4` (the marker constructors that take `CsErrorCode` / `CsMessageKey` / `CsRoleRef` / `CsResourceKeyRef`) |
| **2** | Shared contract | CE-API, CE-VA, CE-ID | **E** | `csrb4` (the `CsOperationRef` parameter) |
...
types and the `tom_core_codespecs` gap classes have all landed, and **no slice is
...
tom_specs-owned item: `csrb4`, the marker constructors, which alone gates
slices 1, 2, 4 and 5 — the ref types exist, but the markers do not yet take them
as parameters. **No `tom_core`-side blocker remains anywhere** — `csexb2` closed
the last one by reconciling the CE-LO container-kind set with the ACL substrate
at slice 6. The implied sequence: the constructors first (`csrb4`), then the two
standing ownership questions (`csrb1`, `csrb2`).
...
vocabulary; the `Cs*` markers that take them get those parameters together with
their other constructor arguments, from the single per-marker table in
[codespecs_derivation_contract.md](codespecs_derivation_contract.md) §5.1
(`csrb4`, §10). Splitting a constructor by argument *type* would leave markers
such as `@CsTrigger`, `@CsAuthorize` and `@CsJob` holding their reference slots
but missing the `required` arms beside them — a surface that reads complete and
is not.
