...
- **`tom_som_dart_v0`** (pkg `tom_som_dart_v0`, `publish_to: none`, generated `lib/tom_som_dart_v0.dart`) is the **typed facade for model major 0**: **3989** classes, each `extends SomNode`, mirroring the 14 roots (`D00SolutionBlueprint` … `D13CodeSpecsProjection`). That is **not** one class per model class: it is one per model class the documents reach (the 1254 in the meta, less the container root `DocSpecsProject`, which is a tree root rather than a document) **plus one per form section** — 2736 of those, since a form's fields are addressed through a class of their own. Typed access reads/writes through the runtime, so it stays a thin, validated projection of the underlying document tree rather than a second copy of state. See `tom_som_dart_v0/example/a_typed_access.dart` (`D00SolutionBlueprint(doc)` over a live document).
...
**Extended tool surface (D4rt scripting, search, memory, files).** [`llm_and_d4rt_tools.md`](llm_and_d4rt_tools.md) adds the in-process `tom_spec_engine` scripting plane, all routed through the **same controller** (so the change log stays the single source — §9). The surface is organised into three D4rt **scopes** binding **five globals**, each reached through its own import, selected by the active application's **AgentContext** (§7.3):

- **`spec` scope** — the only scope binding more than one global, all three over the same live `SpecDocumentController`:
  - `package:tom_spec_engine/spec_api.dart` → `spec`, the **eight-method `SpecApi`** (`content`/`setContent`/`formField`/`setFormField`/`listItems`/`addListItem`/`removeListItem`/`addChild`), so scripted edits are **identical to tool edits** (one change log, undoable). `addChild` is **meta-model-validated** and throws on an illegal child.
  - `package:tom_spec_engine/spec_model_api.dart` → `model`, the read-only `SpecModelApi` reflection surface (`reflect`/`resolves`/`kindOf`/`classOf`/`sectionId`/`mapsTo`/`detailedIn`/`headline`/`allowedChildren`/`annotations`), reading the same meta-model as `doc_reflect`.
  - `package:tom_spec_engine/spec_search_api.dart` → `search`, the read-only `SpecSearchApi` (`grep`/`query`) returning an edit-stable `SpecSearchCursor` (`next`/`take`/`toList`/`count`), over the same query engine as `doc_search`.
...
**`model` and `search` are host-provided.** `specScope()` takes lazy model/query-engine providers and omits the global whose provider returns `null`, so a `script_validate` issued before the document's model has loaded sees only `spec`; `script_run` awaits the load, so a **run** always sees all three. `SpecScriptRunner` supplies both providers.

**Both surfaces carry search and reflection.** In-script they are `search` / `model`; as MCP tools they are `doc_search` / `doc_search_iterate` / `doc_reflect` / `doc_add_node` over the live document, and `model_search` / `model_get_section` / `model_children` over the Layer-A object model. The in-script and tool forms read the same engine and the same meta-model, so they agree by construction — the choice is one answer versus a loop. `script_author` / `script_validate` / `script_run` / `script_list` / `script_get` and `mem_recall` / `mem_refresh` ship alongside them; scripts run through the editor's in-process `SpecScriptRunner` (`AgentToolsModule.scriptRunner`). The one shipped-but-unbound piece is the typed `tom_som_dart_v0` facade — available as the reusable `somBridgedLibrary()` block, not registered by `specScope()` (see `llm_guidelines_specification.md` §10).
...
- **Root navigator** (left panel / top nav), in **two bands**:
  1. **Authored documents** — **Solution Blueprint** first, then a **separator**, then the Phase-3 projection roots **alphabetically by title**.
  2. **Generation inputs** — below a second separator and the heading `Generation inputs`, the generation-input roots, also alphabetically.

  Selecting a root re-renders the document editor as that projection over shared data.
- **Generation input vs authored document.** A **generation input** is an `@Document` root whose reader is a downstream generator rather than a spec author. `D13CodeSpecsProjection` is the only one today: it is `@CodeSpecKind`-driven rather than `@DetailedIn`-driven, flat, grouped by `@Comment('locus: …')`, and carries no document identity anybody would sit down and write. Listing it as a peer of RSP and IFM would state that the spec has fourteen documents to author, when it has thirteen plus one projection the Phase-4 generator consumes.
  - **The classification is derived, never authored twice.** A root is a generation input iff its class carries the `@CodeSpecsProjection()` marker — the same marker that exempts it from the `tom_specs_model_rules.md` §10.2 detail-count check. The export already carries it losslessly in `classes[<type>].annotations`, so the editor reads it through `SpecModel.isGenerationInput` (in `tom_som_dart_runtime`, shared with the reviewer) instead of matching on a root type name. Adding a second generation projection therefore needs no editor change.
  - **The split is presentational, not a permission.** Banded roots stay **selectable and editable**. An edit made through any projection folds onto the same Solution Blueprint storage path (see the storage-path key bullet below), so the identical edit is one click away under the blueprint — a read-only flag here would forbid nothing and merely hide that the projection is a live view. What the band removes is the *category error*, not the capability.
  - **Never the default landing root.** `defaultNavigatorRoot` resolves to the Solution Blueprint, else the first **authored** root, and only falls back to a generation input when the model has nothing else — opening the editor on the CodeSpecs projection would present a generator's input as the thing the user came to write.
  - **The reviewer is deliberately unchanged.** `tom_specs_reviewer` reviews the *object model*, where a generation projection is a legitimate model root to inspect like any other; the band is an authoring-surface concern only.
...
- **Serialization mechanism (N9, N11) — `toYaml` on the model.** Each root class has a `toYaml` method:
  ...
  - **Connect before write, not after read (N11, N12):** a projection root default-constructs its own children, and a referenced SBP section may be **null** — in which case it is null in the projection too (one shared tree, no divergence). Rather than wiring them at load time (which would force keeping copies in sync), the global root runs a **connect pass immediately before** serializing an individual projection root — binding that root's references to **whatever SBP sections currently exist**. So an individual-root write always reflects the live SBP state, and the in-memory projections stay reference-only between writes. Because of the **pure-projection invariant (§14, N12)** — no projection-root content exists outside the SBP — this connect pass never has to invent or drop content; it only re-points references.
  - **How a binding is derived.** The spec-ops codegen (`SpecOpsGenerator`) emits one `connect` per projection root, resolving each child to an SBP field path **by type, then by member name**. Type carries most of it: a section class is declared once and appears in exactly one place under the master, so a projection child of type `T` binds to the single SBP field of type `T`. The member name settles the two cases where type is not unique — the shared `tom_specs_core` **content-leaf section types** (`DiagramSection` and its siblings are not per-section classes), and a **flattened** root that also reaches a class nested under one of its own siblings. Deriving from the declared structure rather than from the `@MapsTo` / `@DetailedIn` seed/promotion pair means the bindings cannot fall out of step with the roots by hand; the traceability annotations continue to state *which* document a section flows to, which is a different question. The one child deliberately left unbound is the **`DocumentHeader`**: each of the fourteen entry points is a document in its own right, with its own id, version and author.
...
20. **Root navigator over the single tree (§14).** Two bands — authored documents (Solution Blueprint first, separator, then the rest alphabetically), then the `Generation inputs` band; editing any projection, banded or not, edits the shared sections. **Done:** switching roots re-renders the same underlying data consistently.
