# Rust tutorial — `tom_som_rust_v0`

Using the TomSpecs Specification Object Model from Rust, end to end:
...
same plane is
[`tom_som_rust_runtime/doc/generic_access.md`](../../tom_som_rust_runtime/doc/generic_access.md).
...
`tom_som_rust_v0` is the **typed** access path: one generated type per document
...
`tom_som_rust_runtime` actually holds — the runtime carries the sparse,
...
together — `tom_som_rust_v0` and `tom_som_rust_runtime` always carry the
...
fn main() {
    // 1 — open a document and wrap it in a typed root. The constructor runs
    // the instantiation-time model-version check (an empty stamp is editable).
    let doc = som::doc_ref(som::SpecDocument::new());
    let bp = D00SolutionBlueprint::new(doc.clone(), "").expect("new root");

    // 2 — edit through named members, not string paths.
    bp.set_content("A platform that unifies our fragmented order systems.");
    bp.current_landscape()
    ...
    let model = som::SpecModel::from_json_str(&data).expect("parse meta");
    println!("{}", som::validate_document(&model, &doc.borrow()).is_empty());

    // 5 — serialize, then read the value back out of the decoded document.
    let tree = meta::d00_solution_blueprint_meta_tree();
    let y = som::encode_yaml(&doc.borrow(), &tree, "1.0").expect("encode");
    let decoded = som::decode_yaml(&y, &tree).expect("decode");
    println!("{}", decoded.document.content("SBP/content").unwrap());
    println!("{}", decoded.model_version);
}
```
...
./tool/regenerate_api_references.sh rust_v0
```

That renders it with `cargo doc`. The reasoning behind not committing it, and
...
Fallible calls return `Result`; nothing panics on a data problem. A document handle is an `Rc<RefCell<SpecDocument>>`, so a typed root and the generic store share one document without either owning it.
...
- **Prefer the typed path.** Reach for the generic store only when the path is
  ...
  [`generic_access.md`](../../tom_som_rust_runtime/doc/generic_access.md).
