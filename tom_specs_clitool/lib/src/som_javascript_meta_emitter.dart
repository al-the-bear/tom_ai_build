/// Emits the generated **metadata module** of the JavaScript `v0` facade
/// (the JavaScript counterpart of `som_dart_meta_emitter.dart` /
...
/// `$` is a legal JavaScript identifier character, so the Dart names carry
/// over verbatim and are collision-free against the model class names):
///
///   * one `class <Class>$Nav extends SomMetaRef` accessor class per model
///     class — getters named exactly after the model members;
///   * one `class <Class>$Id extends SomMetaRef` accessor class per
...
///     `<ROOT-SEGMENT>`. Like Python, a CommonJS module executes top-down, so
///     the root constants are emitted **after** all accessor classes.
///
/// One JS-specific divergence: `SomListMetaRef`'s element parameter is a
/// **factory function** `(tree, path) => ref` (JS classes are not callable
/// without `new`, unlike Python), so list getters pass arrow-function
/// factories instead of bare class references.
library;
...
/// is a JavaScript syntax error). Collisions fail generation loudly.
...
/// Emits the JavaScript source of the generated metadata module (see the
/// library doc above) for [model].
class SomJavaScriptMetaEmitter {
  ...
  String generateLibrary() {
    final b = StringBuffer()
      ..writeln(
        '// GENERATED by tom_specs_clitool SomJavaScriptMetaEmitter '
      ...
      ..writeln(
        '// recorded in this package\'s `package.json` (written by '
        'the generator), so',
      )
  ...
  void _emitCxHelper(StringBuffer b) {
    b
      ...
      ..writeln('function _cx(cls, stack, kids, make) {')
  ...
  /// The JavaScript `SomMetaKind` member for a [SpecFieldKind].
  ...
  /// A JavaScript double-quoted string literal for [s] (full
  /// control-character escaping — doc comments and descriptions carry
  /// newlines).
  String _str(String s) =>
      '"${s.replaceAll('\\', '\\\\').replaceAll('"', '\\"').replaceAll('\n', '\\n').replaceAll('\r', '\\r').replaceAll('\t', '\\t')}"';

  /// A JavaScript literal for a JSON-shaped annotation-argument [value].
  ...
  void _emitMetaChildrenBuilder(StringBuffer b, SpecClass cls) {
    b
      ..writeln('function _mc_${cls.name}(s) {')
      ..writeln('  return [');
  ...
  /// Emits the tree + entry-point constants of [root] and returns the
  /// exported constant names (tree, dot-notation root, ID-tree root).
  List<String> _emitRoot(StringBuffer b, SpecRoot root) {
    ...
    b
      ...
      ..writeln('const ${camel}MetaTree = new SomMetaTree(new SomMetaNode({')
      ..writeln('  ${args.join(',\n  ')},')
      ..writeln('}));')
      ..writeln()
      ...
      ..writeln(
        'const $camel = new ${root.type}\$Nav(${camel}MetaTree, '
        '${_str(seg)});',
      )
      ..writeln()
      ...
      ..writeln(
        'const $idSymbol = new ${root.type}\$Id(${camel}MetaTree, '
        '${_str(seg)});',
      )
      ..writeln();
  ...
  void _emitRootRegistry(StringBuffer b) {
    b
      ...
      ..writeln()
      ..writeln(
        '// Every document root this module generates, keyed by root '
        'class name and in',
      )
      ..writeln(
        '// model order. Each entry carries the root class name, the '
        'path segment its',
      )
      ..writeln(
        '// access roots are bound at, the populated metadata tree '
        '(SOM §7.2), and the',
      )
      ..writeln(
        '// two SOM §8 access roots. Emitted from the same root list '
        'that produced the',
      )
      ..writeln(
        '// trees above, so enumerating this object is equivalent to '
        'reading the',
      )
      ..writeln(
  ...
  String _navGetterExpr(SpecField f, String seg) {
    ...
    switch (f.kind) {
      ...
      case SpecFieldKind.list:
        ...
        return 'new SomListMetaRef($rel, (t, p) => new SomMetaRef(t, p))';
      case SpecFieldKind.form:
      case SpecFieldKind.content:
      case SpecFieldKind.enumValue:
      case SpecFieldKind.scalar:
  ...
  void _emitIdClass(StringBuffer b, SpecClass cls) {
    ...
    for (final child in _idChildren(cls)) {
      ...
      if (child.isList) {
        ...
        expr = elem != null
            ? 'new SomListMetaRef($rel, (t, p) => new $elem\$Id(t, p))'
            : 'new SomListMetaRef($rel, (t, p) => new SomMetaRef(t, p))';
  ...
  /// A section id as a JavaScript identifier: `-` → `_`, digit-leading ids
