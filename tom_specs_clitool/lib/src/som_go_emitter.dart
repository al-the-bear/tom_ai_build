...
class SomGoEmitter {
  ...
  String generateLibrary() {
    ...
    final buffer = StringBuffer()
      ..writeln(
        '// GENERATED by tom_specs_clitool SomGoEmitter '
      ...
      ..writeln('//')
      ..writeln(
        '// The generic runtime is imported through its domain-qualified '
        'module path; the',
      )
      ..writeln(
        '// generator wires local resolution by writing a `go.mod` with a '
        '`require` + local',
      )
      ..writeln(
        '// `replace` directive, so this module builds both standalone '
        '(`go get`) and in-repo.',
      )
      ..writeln('//')
      ..writeln(
      ...
      ..writeln(
        '// dot-notation and ID-tree access surfaces — lives in the '
        'sibling `_meta.go` file',
      )
      ..writeln(
        '// of this same package, so one import surfaces both access '
        'styles.',
      )
    ...
    buffer
      ..writeln()
      ..writeln(
        '// Version is the semantic version of this generated facade '
        'module, matching the',
      )
      ..writeln(
        '// object-model version it was generated against '
        '(vMAJOR.MINOR.PATCH). It is the',
      )
      ..writeln(
        '// in-source counterpart of the VCS tag used to pin the module '
        '(SOM §4.2).',
      )
      ..writeln(
        'const Version = "v${packageVersionFromModel(modelVersionString)}"',
      )
      ..writeln();
  ...
  String _emitClass(
    ...
    if (isRoot) {
      ...
      b
        ..writeln(
          '// $mvConst is the model version this object '
          'model was generated',
        )
        ...
        ..writeln()
        ..writeln(
          '// $ctor creates the typed facade at the document '
        ...
        ..writeln(
          'func $ctor(doc *som.SpecDocument, documentVersion '
          'string) (*${cls.name}, error) {',
        )
        ..writeln(
          '\tif err := som.CheckSomModelVersion($mvConst,'
          ' documentVersion); err != nil {',
        )
        ..writeln('\t\treturn nil, err')
        ..writeln('\t}')
        ..writeln(
          '\treturn &${cls.name}{SomNode: som.NewSomNode(doc, '
          '"${_goStr(seg)}")}, nil',
        )
        ..writeln('}')
        ..writeln()
        ..writeln(
          "// ObjectModelVersion returns this object model's own model "
          'version (major.minor),',
        )
        ..writeln('// per SOM §4.2.')
        ...
        ..writeln()
        ..writeln(
          '// EditabilityFor classifies whether a document authored '
          'under documentVersion',
        )
        ..writeln(
          '// is editable by this object model, without returning an '
          'error (SOM §21) — the',
        )
        ..writeln(
          "// non-error-returning companion to $ctor's SOM §4.2 check, so a "
          'read-only viewer',
        )
        ..writeln('// can branch instead of handling the constructor error.')
        ..writeln(
          'func (x *${cls.name}) EditabilityFor(documentVersion '
        ...
        ..writeln()
        ..writeln(
          '// LoadYaml${cls.name} loads a `*.docspecs.yaml` document in '
          'one call: decode the',
        )
        ..writeln(
          '// YAML, populate the sparse stores, and construct the typed '
          'root at the document',
        )
        ..writeln(
          "// root with the document's retained authoring stamp — one "
          'call for the former',
        )
        ..writeln(
          '// decode → loadJson → thread-documentVersion sequence '
          '(SOM §21). Returns a',
        )
        ..writeln(
          '// *som.SomVersionError when the stamp is not editable '
          '(SOM §4.2). Decoding runs',
        )
        ..writeln(
          '// against the generated ${cls.name}MetaTree, so v2 '
          'hierarchical documents',
        )
        ..writeln(
          '// resolve their section paths through the metadata tree '
          '(SOM §8).',
        )
        ..writeln(
          'func LoadYaml${cls.name}(yaml string) (*${cls.name}, error) '
          '{',
        )
        ...
        ..writeln()
        ..writeln(
          '// LoadFile${cls.name} loads a `*.docspecs.yaml` document '
          'from the file at path —',
        )
        ..writeln(
          '// the file companion to LoadYaml${cls.name}. A read or '
          'decode error is returned',
        )
        ..writeln('// to the caller.')
        ..writeln(
          'func LoadFile${cls.name}(path string) (*${cls.name}, error) '
          '{',
        )
      ...
      b
        ..writeln(
          '// $ctor binds a ${cls.name} facade to a document '
          'and a path.',
        )
        ..writeln(
          'func $ctor(doc *som.SpecDocument, path string) '
          '*${cls.name} {',
        )
    ...
    if (_hasContentLeaf(cls)) {
      b
        ..writeln()
        ..writeln(
          '// CanHaveContent reports that this section type declares the '
          'standard `content`',
        )
        ..writeln(
          '// text leaf (SOM §21) — it shadows the embedded '
      ...
      case SpecFieldKind.list:
        ...
        if (f.elementIsComplex && f.elementType != null) {
          ...
          b
            ...
            ..writeln(
              '\treturn som.NewSomList(x.Doc(), $childPathTight, '
          ...
          b
            ..writeln(
              'func (x *${cls.name}) $acc() *som.SomList[*som.SomScalar] '
              '{',
            )
            ..writeln(
              '\treturn som.NewSomList(x.Doc(), $childPathTight, '
  ...
  String _emitFormClass(String name, SpecField f) {
    ...
    final b = StringBuffer()
      ..writeln(
        '// $name is the generated section facade for the `${f.name}` '
      ...
      ..writeln()
      ..writeln(
        '// CanHaveContent reports that this @Form section holds body '
        'text before its',
      )
      ..writeln(
        '// form fields (SOM §21) — it shadows the embedded '
        'som.SomNode false default.',
      )
    ...
    if (!hasContentMember) {
      ...
      b
        ..writeln()
        ..writeln(
          "// Content is the section's own free-text content, before "
          'the form fields.',
        )
    ...
    switch (_scalarType(ff.type)) {
      case 'int':
        b
          ...
          ..writeln('\t}')
          ..writeln(
            '\tx.Doc().SetFormField(x.Path(), $field, '
            'strconv.Itoa(*value))',
          )
          ..writeln('}');
      case 'double':
