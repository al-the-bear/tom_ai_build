...
void main() {
  ...
  group('SomRustEmitter', () {
    ...
    test('the generated crate cargo-builds clean against the runtime', () {
      ...
      _expectRustBuilds(
        SomRustEmitter(model).generateLibrary(),
    ...
    test(
      ...
      () {
        ...
        expect(
          SomRustEmitter(stamped, versionLabel: 'v0').modelVersionString,
          '1.3',
        );
        expect(
          SomRustEmitter(stamped, versionLabel: 'v9').modelVersionString,
          '1.3',
        );
      },
    );
    ...
    test('enum tokens are preserved as module-level constants', () {
      ...
      expect(
        source,
        contains('pub const PROBABILITY_MEDIUM: &str = "medium";'),
      );
      expect(source, contains('pub const PROBABILITY_HIGH: &str = "high";'));
      expect(
        source,
        contains('pub fn parse_probability(token: &str) -> String'),
      );
    });
    ...
    test('a pattern-bearing list emits its @SectionIdPattern; a scalar list '
      ...
      expect(
        RegExp(
          r'pub fn risks\(&self\) -> som::SomList<Risk>[\s\S]*?'
    ...
    test('the one-call loaders thread the generated metadata tree through the '
      ...
      expect(
        source,
        contains(
          'som::SpecDocument::from_yaml(yaml, &tree)'
      ...
      expect(
        source,
        contains(
          'som::SpecDocument::from_file(path, &tree)'
          '.map_err(SomLoadError::Yaml)?;',
        ),
      );
