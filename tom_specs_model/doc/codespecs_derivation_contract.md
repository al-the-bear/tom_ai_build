...
| Point | Contract |
|-------|----------|
| **1 Input** | `SCHMG` / `SCMST`. §8.5 records CE-MG as a **GAP** (`csrb11`) — the SOM home does not yet carry the §5.27 artifact surface; this entry specifies the contract, `csrb11` supplies the fields. Additional input: the §3.3.1/§3.3.2 entity model, which the cumulative DDL must converge on. |
...
| **3 Arguments** | `datasource` and `schema` (**both required**) ← the artifact tree's placement, verbatim — they are the directory path and cannot be defaulted without silently retargeting a database. `kind` (**required**) ← `CsMigrationKind {initialDdl, baseData, iteration}`, §5.27's three artifact kinds; no arm is a safe default, since generating an initial DDL where an iteration was meant would rewrite a live schema. Filenames are **not** arguments — they are file-system facts and one of the four §5.23 string exemptions. |
| **4 Naming** | Declaration = `<Datasource><Schema>Migrations`, PascalCase. **Artifact filenames are authored, never derived** (N5): the version is the author's ordering decision and a derived one would renumber on every spec edit. |
| **5 Locus** | `server` — the migrations tree ships with the server project as file assets. |
| **6 Cross-refs** | None typed. The convergence obligation — cumulative DDL vs. the `@CsTable`/`@CsColumn` model — is a **named generation-time validator check**, precisely because the filename exemption means it cannot be a compile-time edge. |
...
| Point | Contract |
|-------|----------|
| **1 Input** | §8.5 records CE-CF as a **GAP** (`csrb9`) — there is no SOM configuration section yet; `csrb9` creates it for all four scopes. Consumed once it exists (§5.16): the setting's key, type and default, plus source key / precedence per §5.5. |
...
| **6 Cross-refs** | None typed. Retention, log format and the compliance report land here rather than on CE-LG — they are sink deployment settings. |
...
| Point | Contract |
|-------|----------|
| **1 Input** | §8.5 records CE-CL as a **GAP** (`csrb13`) — there is no SOM section declaring which clients exist. This entry fixes the contract so the part is ready the moment `csrb13` lands. Consumed once it exists: the client's id, its kind, its target platforms. |
| **2 Output** | A **client descriptor** — a plain annotated model class (form 2). §4.1.1 records that CE-CL has **no `tom_core` basis by design**: it is the one part with nothing to build on, because the framework has no notion of "which apps exist". |
| **3 Arguments** | `clientId` — **first positional, required**, verbatim. `kind` (**required**) ← `CsClientKind {flutterApp, cli, server}`, §4.1's three client kinds; required because the kind decides which other parts the client can carry (a CLI has no CE-EL) and defaulting it would silently admit impossible combinations. Platforms are members of the descriptor (test **a**). |
...
| **6 Cross-refs** | `CsRouteRef` for its entry route. Referenced **by** CE-CC (per-client configuration) and CE-AU (per-client login flow). |
| **7 Back-link** | `@DocSpec([DocRef('<csrb13 section id>', 'supplies the client application and its kind')])`. |
...
| Point | Contract |
|-------|----------|
| **1 Input** | `BatchJobManagement` (`BAJOMA`) — §8.5 records CE-JB as a **GAP** (`csrb12`); this entry fixes the contract, `csrb12` supplies the fields. Consumed (§5.29): the trigger, the work definition, the target references, the retry/backoff/timeout/alerting policy. |
| **2 Output** | A `TomJobDeclaration` (`tom_core_codespecs` **gap class** — the job base the substrate lacks, making execution pluggable into any scheduling system) whose work body is **form-3 compilable pseudo-code over a later-injected abstract service class**, dispatched on the `TomCommand` / `TomExecutor` / `TomWorker` isolate-pooling substrate (`tom_core_kernel`). `TomJobDeclaration` carries `enabled` (opt-out), `environments` (empty = every environment), `serviceUnitId` and `targetRefs` on its constructor (test **b**). |
| **3 Arguments** | `trigger` (**required**) ← `CsJobTrigger {cron, calendar, event}`, §5.29's three trigger kinds; required because a job with no declared trigger is a job that never runs. Per-kind slots (§2.3): `cron`, `calendar`, `event` — each a verbatim authored string, and each meaningless for the other two kinds. Policy: `maxRetries` (default `0` — a job retries only when the spec says so), `backoff` and `timeout` as `Duration` consts, `failureAlert` as a `CsMessageKey`. Until a scheduler runtime lands, a trigger is a **named-but-unwired** schedule; that is a `tom_core` roadmap item, not a CodeSpecs gap. |
| **4 Naming** | PascalCase of `BAJOMA`'s job-name field + `Job`. |
| **5 Locus** | `server` — work that runs *off* the request thread is the axis separating CE-JB from CE-API. |
| **6 Cross-refs** | Emits `CsJobRef`. Cites `CsServiceUnitRef`, `CsReportRef`, `CsMessageKey`, and its target entities by `Type` — typed const refs, never strings (§5.29). |
| **7 Back-link** | `@DocSpec([DocRef('BAJOMA', 'supplies the job, its trigger and its target set')])`. |

#### 3.7.2 `@CsAudited` — CE-LG audited element

| Point | Contract |
|-------|----------|
| **1 Input** | `SecurityEventEntry` (`SEVT`), `AuditLogFormat` (`AULOFO`), `EventAttributePolicy` (`EVATPO`) — §8.5 records CE-LG as COVERED but **narrow** (`csrb5`). Consumed: which invocations are auditable, whether reads count, which fields must never appear. |
...
| **3 Arguments** | None; `@CsAudited({String? note})` unchanged. The three authored decisions are **exactly** `@TomAudited`'s three parameters (test **b**) — the same pattern CE-SU uses with `@tomService`. Retention, log format and the compliance report are **not** CE-LG: they are sink deployment settings and belong to `@CsServerConfig` (§3.3.6). |
...
| `@CsJob` | `{required CsJobTrigger trigger, String? cron, String? calendar, String? event, int maxRetries = 0, Duration? backoff, Duration? timeout, CsMessageKey? failureAlert}` |
