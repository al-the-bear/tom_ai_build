/// Emits the generated **metadata module** of the Python `v0` facade (the
/// Python counterpart of `som_dart_meta_emitter.dart`): the
...
const Set<String> _reservedAccessorNames = {'path', 'meta', 'tree', 'item'};
...
class SomPythonMetaEmitter {
  ...
  String generateLibrary() {
    final b = StringBuffer()
      ..writeln(
        '# GENERATED by tom_specs_clitool SomPythonMetaEmitter '
  ...
  void _emitCxHelper(StringBuffer b) {
    b
      ...
      ..writeln(
        '    """Builds a complex/section (or list-element) node with '
        "the bridge's",
      )
      ..writeln(
        '    cycle rule: a class already on the descent *stack* '
        'becomes a terminal',
      )
  ...
  void _emitRootRegistry(StringBuffer b) {
    b
      ...
      ..writeln()
      ..writeln("    __slots__ = ('type', 'segment', 'tree', 'nav', 'id')")
      ..writeln()
      ..writeln('    def __init__(self, type, segment, tree, nav, id):')
      ..writeln(
        '        #: The root class name, e.g. '
        '``D00SolutionBlueprint``.',
      )
      ..writeln('        self.type = type')
      ..writeln(
        '        #: The path segment the two access roots are bound '
        'at — the root',
      )
      ..writeln(
        '        #: section id where the model gives one, else the '
        'class name.',
      )
      ..writeln('        self.segment = segment')
      ..writeln(
        '        #: The populated metadata tree of this root '
        '(SOM §7.2).',
      )
      ..writeln('        self.tree = tree')
      ..writeln(
        '        #: The dot-notation access root '
        '(a ``SomMetaRef``).',
      )
      ..writeln('        self.nav = nav')
      ..writeln('        #: The ID-tree access root (a ``SomMetaRef``).')
      ..writeln('        self.id = id')
      ..writeln()
      ..writeln()
      ..writeln(
      ...
      ..writeln(
        '#: and access roots above, so enumerating this mapping is '
        'equivalent to',
      )
      ..writeln(
        '#: reading the generator input — no consumer needs a '
        'hand-kept copy of the',
      )
      ..writeln('#: root set.')
      ..writeln('SOM_META_ROOTS = {');
    for (final root in _roots) {
      final seg = _rootSegment(root);
      final camel = _camel(root.type);
      b.writeln(
        '    ${_str(root.type)}: SomMetaRootEntry('
        '${_str(root.type)}, ${_str(seg)}, ${camel}MetaTree, $camel, '
        '${_idName(seg)}),',
      );
    }
    b
  ...
  void _emitNavClass(StringBuffer b, SpecClass cls) {
    b
      ...
      ..writeln(
        '    one navigable position: ``.path`` is the absolute '
        'document path, ``.meta``',
      )
      ..writeln(
        '    the metadata node. Past a recursive re-entry ``.path`` '
        'chains remain valid',
      )
      ..writeln(
        '    document positions while ``.meta`` raises (the metadata '
        'tree ends there)."""',
      );
    for (final f in _orderedFields(cls)) {
  ...
  void _emitIdClass(StringBuffer b, SpecClass cls) {
    b
      ...
      ..writeln(
        '    """ID-tree accessors of ``${cls.name}`` (SOM §8): '
        'getters named by',
      )
      ..writeln(
        '    section id (``-`` → ``_``), hoisted through id-less '
        'members so every',
      )
      ..writeln(
