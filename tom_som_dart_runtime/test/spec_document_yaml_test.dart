...
''';
  ...
  // YRD6: title/id role fields never appear in the yaml form block — the item
  // key IS the id-role value and the `headline` key IS the title-role value.
  // A role key inside a form mapping is a format error, both ways.
  group('YRD6 — title/id role form fields', () {
    final cardsModel = SpecModel.fromJson({
      'roots': [
        {'type': 'CardDoc', 'title': 'Card Doc', 'sectionId': 'C00'},
      ],
      'classes': {
        'CardDoc': {
          'name': 'CardDoc',
    ...
    final cardsTree = buildSomMetaTree(cardsModel);

    SpecDocument populated() {
      final d = SpecDocument();
      final c1 = d.addListItem('C00/CARD-LST');
    ...
    test('role values emit once — item key + headline, no form entry', () {
      final yaml =
          SpecDocumentYaml.encode(document: populated(), tree: cardsTree);
      expect(yaml, contains('CARD-ALPHA:'));
      expect(yaml, contains('headline:'));
      expect(yaml, contains('Alpha Card'));
      expect(yaml, contains('first card'));
      expect(yaml, isNot(contains('cardId')));
      expect(yaml, isNot(contains('name:')));
    });

    test('round-trip restores the role-field views from the stores', () {
      final yaml =
          SpecDocumentYaml.encode(document: populated(), tree: cardsTree);
      final decoded = SpecDocumentYaml.decode(yaml, cardsTree).document;
      expect(decoded.itemSectionId('C00/CARD-LST-1'), 'CARD-ALPHA');
      expect(decoded.headline('C00/CARD-LST-1'), 'Alpha Card');
      expect(decoded.formField('C00/CARD-LST-1/content', 'note'), 'first card');
      expect(SpecDocumentYaml.encode(document: decoded, tree: cardsTree), yaml);
    });

    test('decode rejects a role key written as a form entry', () {
      const yaml = 'version: 2\n'
          ...
          '        note: kept\n';
      expect(
        () => SpecDocumentYaml.decode(yaml, cardsTree),
        throwsA(isA<SpecYamlFormatException>()),
      );
    });

    test('encode rejects a role value smuggled into the form store', () {
      final d = populated();
      // Corrupt the store below the public API: loadJson bypasses the facade.
      final state = d.toJson();
      ((state['forms'] as Map)['C00/CARD-LST-1/content'] as Map)['cardId'] =
          'CARD-DUP';
      final corrupt = SpecDocument()..loadJson(state.cast());
      expect(
        () => SpecDocumentYaml.encode(document: corrupt, tree: cardsTree),
        throwsA(isA<SpecYamlFormatException>()),
      );
    });
  });
