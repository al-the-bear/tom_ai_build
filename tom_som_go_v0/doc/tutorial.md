# Go tutorial — `tom_som_go_v0`

Using the TomSpecs Specification Object Model from Go, end to end:
...
same plane is
[`tom_som_go_runtime/doc/generic_access.md`](../../tom_som_go_runtime/doc/generic_access.md).
...
`tom_som_go_v0` is the **typed** access path: one generated type per document
...
`tom_som_go_runtime` actually holds — the runtime carries the sparse,
...
together — `tom_som_go_v0` and `tom_som_go_runtime` always carry the
...
func main() {
	...
	y, err := som.EncodeYaml(doc, somv0.D00SolutionBlueprintMetaTree, "1.0")
	if err != nil {
		panic(err)
	}
	decoded, err := som.DecodeYaml(y, somv0.D00SolutionBlueprintMetaTree)
	if err != nil {
		panic(err)
	}
	// The generic store returns comma-ok; the typed facade returns a plain
	// string. Same value, different idiom — see the note below.
	value, _ := decoded.Document.Content("SBP/content")
	fmt.Println(value)
	fmt.Println(decoded.ModelVersion)
}
```
...
./tool/regenerate_api_references.sh go_v0
```
...
- **Prefer the typed path.** Reach for the generic store only when the path is
  ...
  [`generic_access.md`](../../tom_som_go_runtime/doc/generic_access.md).
