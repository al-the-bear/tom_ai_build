...
`tom_som_dart_v0` is the typed TomSpecs object-model facade for Dart. It depends on `tom_som_dart_runtime`. Both are versioned to the TomSpecs **model version** (currently `1.3.1`) — pin to that version so your document reads and writes match the model the facade was generated from.
...
Both `tom_som_dart_runtime` and `tom_som_dart_v0` carry version `1.3.1`, taken from the TomSpecs model version. When you upgrade the model, regenerate and move to the new matching version so the facade and your stored documents stay in step.
