...
| **6 Cross-refs** | Emits `CsJobRef`. Cites `CsServiceUnitRef`, `CsReportRef`, `CsMessageKey`, and its target entities by `Type` — typed const refs, never strings (§5.29). `TomJobDeclaration.targetRefs` is still `List<String>` and does not yet honour that; tracked as `csrc1`. |
