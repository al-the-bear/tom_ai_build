...
class SomJavaEmitter {
  ...
  String generateLibrary() {
    ...
    final buffer = StringBuffer()
      ..writeln(
        '// GENERATED by tom_specs_clitool SomJavaEmitter '
    ...
    // The flat path-constant holders are retired: the
  ...
  String _emitClass(
    ...
    if (isRoot) {
      ...
      b
        ..writeln(
          '$_i2// The model version this object model was generated '
          'against (SOM §4.2).',
        )
        ..writeln(
          '${_i2}public static final String MODEL_VERSION = '
          '"$modelVersionString";',
        )
        ..writeln()
        ..writeln(
          '$_i2// Creates the typed facade over doc at the document '
          'root and verifies the',
        )
        ..writeln(
          "$_i2// document's authoring documentVersion is editable "
          '(SOM §4.2).',
        )
        ..writeln(
          '${_i2}public ${cls.name}(SpecDocument doc, '
          'String documentVersion) {',
        )
        ..writeln('${_i3}super(doc, "${_jstr(seg)}");')
        ..writeln(
          '${_i3}SomFacade.checkModelVersion(MODEL_VERSION, '
          'documentVersion);',
        )
        ..writeln('$_i2}')
        ..writeln()
        ...
        ..writeln()
        ..writeln(
          '$_i2// Loads a *.docspecs.yaml document and returns the typed '
          'root with the',
        )
        ..writeln(
          "$_i2// document's authoring stamp already applied (SOM §21) "
          '— one call for',
        )
        ..writeln(
          '$_i2// the former decode → loadJson → thread-documentVersion '
          'sequence. The',
        )
        ..writeln(
          "$_i2// root's generated metadata tree is threaded into the "
          'runtime,',
        )
        ..writeln(
          '$_i2// keying the YAML mapping off SomMetaTree instead of '
          'the meta-JSON model.',
        )
        ..writeln('${_i2}public static ${cls.name} loadYaml(String yaml) {')
        ..writeln(
          '${_i3}SpecDocument doc = SpecDocument.fromYaml(yaml, '
        ...
        ..writeln()
        ..writeln(
          '$_i2// Loads a *.docspecs.yaml document from the file at '
          'path — the file',
        )
        ..writeln('$_i2// companion to loadYaml.')
        ..writeln('${_i2}public static ${cls.name} loadFile(String path) {')
        ..writeln(
          '${_i3}SpecDocument doc = SpecDocument.fromFile(path, '
          'TomSomV0Meta.${cls.name}MetaTree);',
        )
        ..writeln('${_i3}return new ${cls.name}(doc, doc.modelVersion());')
        ..writeln('$_i2}')
        ..writeln()
        ..writeln(
          "$_i2// This object model's own model version (major.minor), "
          'per SOM §4.2.',
        )
        ...
        ..writeln()
        ..writeln(
          '$_i2// Classifies whether a document authored under '
          'documentVersion is',
        )
        ..writeln(
          '$_i2// editable by this object model, without throwing '
          '(SOM §21) — the',
        )
        ..writeln(
          "$_i2// non-throwing companion to the constructor's SOM §4.2 "
          'check, so a read-only',
        )
        ..writeln(
          '$_i2// viewer can branch instead of catching '
          'SomVersionError.',
        )
        ..writeln(
          '${_i2}public static SomEditability editabilityFor('
          'String documentVersion) {',
        )
        ..writeln(
          '${_i3}return SomFacade.somEditabilityFor(MODEL_VERSION, '
          'documentVersion);',
        )
        ..writeln('$_i2}');
    } else {
      ...
      case SpecFieldKind.list:
        ...
        if (f.elementIsComplex && f.elementType != null) {
          ...
          b
            ...
            ..writeln(
              '${_i3}return new SomList<>(doc, $childPath, '
  ...
  String _emitFormClass(String name, SpecField f) {
    ...
    final b = StringBuffer()
      ..writeln(
        '$_i1// Generated section facade for the `${f.name}` @Form '
    ...
    switch (_scalarType(ff.type)) {
      case 'int':
        b
          ...
          ..writeln(
            '${_i3}try { return Integer.parseInt(v); } '
      ...
      case 'num':
        b
          ...
          ..writeln('${_i3}if (v == null || v.isEmpty()) return null;')
          ..writeln(
            '${_i3}try { return Double.parseDouble(v); } '
            'catch (NumberFormatException e) { return null; }',
          )
          ..writeln('$_i2}')
          ..writeln()
