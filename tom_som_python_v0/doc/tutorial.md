# Python tutorial — `tom_som_python_v0`

Using the TomSpecs Specification Object Model from Python, end to end:
...
same plane is
[`tom_som_python_runtime/doc/generic_access.md`](../../tom_som_python_runtime/doc/generic_access.md).
...
`tom_som_python_v0` is the **typed** access path: one generated type per document
...
`tom_som_python_runtime` actually holds — the runtime carries the sparse,
...
together — `tom_som_python_v0` and `tom_som_python_runtime` always carry the
...
sys.path.insert(0, os.path.abspath("../tom_som_python_runtime"))

import tom_som_python_v0 as m
from tom_som_runtime import (SpecDocument, SpecModel, validate_document,
...
print(len(validate_document(model, doc)) == 0)

# 5 — serialize, then read the value back out of the decoded document.
y = yaml_encode(doc, d00SolutionBlueprintMetaTree, model_version="1.0")
decoded = yaml_decode(y, d00SolutionBlueprintMetaTree)
print(decoded.document.content("SBP/content"))
print(decoded.model_version)
```
...
./tool/regenerate_api_references.sh python_v0
```

That renders it with `pdoc`. The reasoning behind not committing it, and
...
Failures raise. `validate_document` returns a list of errors rather than raising; a malformed `*.docspecs.yaml` raises with the path it could not place.
...
- **Prefer the typed path.** Reach for the generic store only when the path is
  ...
  [`generic_access.md`](../../tom_som_python_runtime/doc/generic_access.md).
