...
Future<void> main(List<String> arguments) async {
  final parser = ArgParser()
    ..addOption(
      'os',
      help:
          'Flutter build target: linux | windows | macos. '
    ...
    ..addOption(
      'editor',
      help:
          'Path to the tom_specs_editor Flutter package. '
          'Default: <container>/tom_forge/tom_specs_editor.',
    )
    ..addOption(
      'summaries',
      help:
          'Optional directory of pre-generated Dart-editor summary assets '
          'to embed (B1). Skipped with a notice when absent.',
    )
    ..addFlag(
      'generate-summaries',
      help:
          'Generate the B1 analyzer summaries in-place by running '
    ...
    ..addFlag(
      'no-flutter-build',
      help:
          'Run steps 1–6 (versioner, spec-ops, json, schemas, summaries, '
  ...
  final editorDir = p.normalize(
    args.option('editor') ?? p.join(container, 'tom_forge', 'tom_specs_editor'),
  );
  ...
  if (p.normalize(modelJsonOut) != targetJsonOut) {
    _fail(
      '--editor points at $editorDir, whose asset '
  ...
  for (final f in schemaOut.listSync().whereType<File>().where(
    (f) => f.path.endsWith('.docspecs-schema.yaml'),
  )) {
  ...
  if (args.flag('generate-summaries')) {
    ...
    final bundlerDir = p.join(
      container,
      'tom_forge',
      'tom_dart_editor_bundler',
    );
    final editorBuildkit = p.join(editorDir, 'buildkit.yaml');
    if (!File(editorBuildkit).existsSync()) {
      _fail(
        'No buildkit.yaml in $editorDir — the summary scopes are declared '
    ...
    for (final dir in _bundlerPackageDirs(editorBuildkit)) {
      if (!Directory(dir).existsSync()) {
        _fail(
          'buildkit.yaml names a summary package dir that does not exist: '
          '$dir',
        );
      }
    ...
    stdout.writeln(
      '  → bundler wrote the summary assets and '
      'lib/generated/summary_scopes.g.dart under $editorDir',
    );
  } else if (summaries == null) {
    stdout.writeln(
      '  (no --summaries / --generate-summaries given; nothing to '
  ...
  final schemaCount = schemaOut.existsSync()
      ...
      : 0;
  stdout.writeln(
    '  → spec_model.json + $schemaCount bundled schema file(s) staged',
  );
...
List<String> _bundlerPackageDirs(String buildkitPath) {
  ...
  if (block is! Map) {
    _fail(
      '$buildkitPath has no `dart-editor-bundler` block; there are no '
