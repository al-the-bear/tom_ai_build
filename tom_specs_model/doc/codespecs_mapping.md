...
active part COVERED. Two entries are open, both **model** gaps standing behind a
**required** marker argument: one argument resolves against no registry key, the
other against no authored citation at all, so in both cases the value reaching
the generated code is a spelling or a guess rather than a reference. Nothing is
open against the **document** — §1.1.1 fixes the Phase-4 contract, §1.1.2 the
run procedure and §1.1.3 the generated todo tree the procedure instantiates.
