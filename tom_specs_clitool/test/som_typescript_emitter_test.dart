...
void main() {
  ...
  group('SomTypeScriptEmitter', () {
    test('emitted output matches the committed golden files', () {
      ...
      final metaSource = SomTypeScriptMetaEmitter(
        _fixtureModel(),
      ).generateLibrary();
    ...
    test('the generated module tsc-compiles clean against the runtime types', () {
      ...
      try {
        File(
          p.join(dir.path, 'tom_som_typescript_v0.ts'),
        ).writeAsStringSync(source);
        // The facade imports its sibling meta module (SOM §8).
        File(
          p.join(dir.path, 'tom_som_typescript_v0_meta.ts'),
        ...
        final runtimeTypes = p
            .join(runtimeDir, 'node_modules', '@types')
            .replaceAll('\\', '/');
    ...
    test(
      ...
      () {
        ...
        expect(
          SomTypeScriptEmitter(stamped, versionLabel: 'v0').modelVersionString,
          '1.3',
        );
        expect(
          SomTypeScriptEmitter(stamped, versionLabel: 'v9').modelVersionString,
          '1.3',
        );
      },
    );
    ...
    test('TS reserved words are sanitised in accessors, '
      ...
      final source = SomTypeScriptEmitter(
        SpecModel.fromJson(_keywordJson()),
      ).generateLibrary();
    ...
    test('colliding form-class names are disambiguated (no duplicates)', () {
      ...
      final decls = RegExp(
        r'export class (\w+) extends SomNode',
        multiLine: true,
      ...
      expect(
        source,
        contains(
          'export class MigrationRisksGovernanceContentForm '
          'extends SomNode',
        ),
      );
      expect(
        source,
        contains(
          'export class MigrationRisksGovernanceContentForm2 '
          'extends SomNode',
        ),
      );
    });
    ...
    test('documentRoots subsets the generated classes', () {
      ...
      expect(
        all,
        contains('export class CurrentLandscapeAssessment extends SomNode'),
      );

      final justRoot = SomTypeScriptEmitter(
        _fixtureModel(),
    ...
    test('a pattern-bearing list emits its @SectionIdPattern; a scalar list '
      ...
      expect(
        RegExp(
          r'get risks\(\): SomList<Risk> \{[\s\S]*?new SomList\('
    ...
    test('no flat path-constant holder is emitted; the meta module is '
      ...
      expect(source, contains("export * from './tom_som_typescript_v0_meta';"));
      expect(
        source,
        contains(
