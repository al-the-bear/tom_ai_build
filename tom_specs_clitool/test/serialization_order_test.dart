...
''');
      ...
      final out = File(
        p.join(pkg.path, 'lib', 'src', 'model.dart'),
      ).readAsStringSync();
      ...
      expect(
        out.indexOf('@SerializationOrder(0)'),
        lessThan(out.indexOf('String a')),
      );
      expect(
        out.indexOf('@SerializationOrder(1)'),
...
''');
      ...
      final result = stampSerializationOrder(
...
''');
    ...
    test('SD2-U6: missing lib/src throws StateError', () {
      ...
      expect(
        () => stampSerializationOrder(packagePath: dir.path),
  ...
  group('unstampedMembers', () {
    ModelField stamped(String name, int order) => ModelField(
      ...
      typeName: 'String',
      annotations: [
        AnnotationData('SerializationOrder', {'order': order}),
      ],
    );
    ModelField bare(String name) => ModelField(name: name, typeName: 'String');
    ...
    test(
      'SD2-U8: any un-stamped member is flagged as Class.member, sorted',
      () {
        final classes = {
          'Beta': ModelClass(
            name: 'Beta',
