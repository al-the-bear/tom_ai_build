...
/// The Dart source of the **catalogue accessor** library that ships beside the
/// JSON, `tom_specs_model/lib/codespecs_areas.dart`.
///
/// ## Why an accessor at all
///
/// `codespecs_areas.json` ships as *data*, so reaching it from a consuming
/// project meant resolving the package's own directory layout by hand:
///
/// ```dart
/// final lib = await Isolate.resolvePackageUri(
///     Uri.parse('package:tom_specs_model/tom_specs_model.dart'));
/// final text = File.fromUri(lib!.resolve(
///     '../generated-doc/codespecs/codespecs_areas.json')).readAsStringSync();
/// ```
///
/// That is the walk `somSpecModel` was written to delete, and it fails the same
/// way: `Isolate.resolvePackageUri` **returns `null` once no
/// `.dart_tool/package_config.json` is reachable** — the normal state of a
/// shipped AOT binary — and a `null` resolution reads no file without saying
/// so. The payload here is 16 kB rather than 6.4 MB, so unlike the model
/// accessor this one is cheap enough to have no opt-out argument at all; it is
/// still its own library rather than part of the model barrel, because a
/// catalogue of CodeSpecs areas is not part of the object model.
///
/// [json] is the catalogue text exactly as written to
/// `generated-doc/codespecs/codespecs_areas.json`, so the embedded copy and the
/// shipped file cannot disagree.
String areasAccessorLibrary(String json) {
  final b = StringBuffer();
  b.writeln(
    '// GENERATED by tom_specs_clitool bin/codespecs_areas.dart '
    '— do not edit by hand.',
  );
  b.writeln('//');
  b.writeln('// The CodeSpecs area-catalogue accessor. The catalogue is');
  b.writeln('// embedded rather than read from');
  b.writeln('// `generated-doc/codespecs/codespecs_areas.json` because a');
  ...
  b.writeln();
  b.writeln("import 'package:tom_som_dart_runtime/tom_som_dart_runtime.dart';");
  b.writeln();
  b.writeln('/// The CodeSpecs area catalogue this package ships.');
  b.writeln('///');
  b.writeln('/// The 27 areas transcribed from `codespecs_mapping.md` §4.1 +');
  b.writeln('/// §4.4.3 + §4.4.6 — the input every runtime\'s');
  b.writeln('/// `spec_codespecs_extract` surface reads.');
  b.writeln('///');
  b.writeln('/// Parsed on first use and cached, so importing this library');
  b.writeln('/// costs nothing until the catalogue is actually read.');
  b.writeln('///');
  b.writeln('/// ```dart');
  b.writeln("/// import 'package:tom_specs_model/codespecs_areas.dart';");
  b.writeln('///');
  b.writeln('/// final extract = extractCodeSpecs(doc, model,');
  b.writeln('///     catalog: somCodeSpecsAreas);');
  b.writeln('/// ```');
  b.writeln('///');
  b.writeln('/// This is the same content as the shipped');
  b.writeln('/// `generated-doc/codespecs/codespecs_areas.json`; that file');
  b.writeln('/// remains the interchange artifact for the eight non-Dart');
  b.writeln('/// runtimes.');
  b.writeln('CodeSpecsAreaCatalog get somCodeSpecsAreas =>');
  b.writeln('    _somCodeSpecsAreas ??= CodeSpecsAreaCatalog.fromJson(');
  b.writeln(
    '        jsonDecode(somCodeSpecsAreasJson) as Map<String, dynamic>);',
  );
  b.writeln();
  b.writeln('CodeSpecsAreaCatalog? _somCodeSpecsAreas;');
  b.writeln();
  b.writeln('/// The embedded catalogue, as the exact JSON of the shipped');
  b.writeln('/// `generated-doc/codespecs/codespecs_areas.json`.');
  b.writeln('///');
  b.writeln('/// Exposed because a consumer that wants to hand the payload to');
  b.writeln('/// something other than [CodeSpecsAreaCatalog] — another');
  b.writeln('/// language runtime over a pipe, a file it writes out — should');
  b.writeln('/// not have to re-encode a parsed catalogue to get it back.');
  b.writeln('const String somCodeSpecsAreasJson = ${_dartLiteral(json)};');
  return b.toString();
}
...
/// introduced — then the quote, `$` (which would start an interpolation in a
/// non-raw literal) and the newline, so the literal stays on one line.
String _dartLiteral(String s) {
  final e = s
      .replaceAll(r'\', r'\\')
      .replaceAll("'", r"\'")
      .replaceAll(r'$', r'\$')
