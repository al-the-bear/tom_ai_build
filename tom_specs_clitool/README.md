...
| `validate_codespecs.dart` | Run the `codespecs_derivation_contract.md` §6 checks over a generated CodeSpecs project trio. Takes `--shared` / `--client` / `--server`; exits `0` clean, `1` on any violation, `2` on bad usage. The pass has **two** inputs: the trio, and — via `--extracts` — the run's `generated-doc/codespecs_extracts` tree, which is the specification text the three comment checks hold the emitted doc comments against. Each optional input names on stdout the checks it left unrun, so a skipped check never reads as a passed one. |
