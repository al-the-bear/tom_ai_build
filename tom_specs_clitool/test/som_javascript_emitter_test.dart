...
void main() {
  ...
  group('SomJavaScriptEmitter', () {
    test('emitted output matches the committed golden files', () {
      ...
      final metaSource = SomJavaScriptMetaEmitter(
        _fixtureModel(),
      ).generateLibrary();
    ...
    test('the generated module loads under node and a typed mutation '
      ...
      try {
        ...
        File(
          p.join(dir.path, '${moduleName}_meta.js'),
        ...
        final runtimeRel = p
            .relative(runtimeDir, from: dir.path)
            .replaceAll(r'\', '/');
        File(p.join(dir.path, 'package.json')).writeAsStringSync(
          jsonEncode({
            'name': moduleName,
            'version': '0.0.0',
            'private': true,
...
''';
    ...
    test(
      ...
      () {
        ...
        expect(
          SomJavaScriptEmitter(stamped, versionLabel: 'v0').modelVersionString,
          '1.3',
        );
        expect(
          SomJavaScriptEmitter(stamped, versionLabel: 'v9').modelVersionString,
          '1.3',
        );
      },
    );
    ...
    test('colliding form-class names are disambiguated (no duplicates)', () {
      ...
      expect(
        source,
        contains('class MigrationRisksGovernanceContentForm2 extends SomNode'),
      );
    });
    ...
    test('a pattern-bearing list emits its @SectionIdPattern; a scalar list '
      ...
      expect(
        RegExp(
          r'get risks\(\) \{[\s\S]*?new SomList\(this\.doc,[\s\S]*?'
          r'"RISK-ITEM-xxx"\);',
        ).hasMatch(source),
        isTrue,
    ...
    test('no flat path-constant holder is emitted; the meta module is '
      ...
      // codec.
      expect(source, contains("require('./tom_som_javascript_v0_meta.js');"));
    ...
    test('documentRoots subsets the generated classes', () {
      ...
      final justRoot = SomJavaScriptEmitter(
        _fixtureModel(),
        documentRoots: ['SolutionBlueprint'],
      ).generateLibrary();
      expect(
        justRoot,
        contains('class CurrentLandscapeAssessment extends SomNode'),
      );
    });
  });
}
