# TypeScript tutorial — `tom_som_typescript_v0`

Using the TomSpecs Specification Object Model from TypeScript, end to end:
...
same plane is
[`tom_som_typescript_runtime/doc/generic_access.md`](../../tom_som_typescript_runtime/doc/generic_access.md).
...
`tom_som_typescript_v0` is the **typed** access path: one generated type per document
...
`tom_som_typescript_runtime` actually holds — the runtime carries the sparse,
...
together — `tom_som_typescript_v0` and `tom_som_typescript_runtime` always carry the
...
import {
  SpecDocument, SpecModel, validateDocument, yamlEncode, yamlDecode,
} from 'tom_som_typescript_runtime';
import { D00SolutionBlueprint } from './tom_som_typescript_v0';
import { d00SolutionBlueprintMetaTree } from './tom_som_typescript_v0_meta';
...
const y = yamlEncode(doc, d00SolutionBlueprintMetaTree, '1.0');
const decoded = yamlDecode(y, d00SolutionBlueprintMetaTree);
...
```bash
npm run build && node dist/tutorial.js
```
...
./tool/regenerate_api_references.sh typescript_v0
```
...
- **Prefer the typed path.** Reach for the generic store only when the path is
  ...
  [`generic_access.md`](../../tom_som_typescript_runtime/doc/generic_access.md).
