 ...
 * `spec_document_yaml.py` / `SpecDocumentYaml.java`).
 *
...
function _roundTrips(block, value) {
  try {
    ...
    return parsed !== null && typeof parsed === 'object' && parsed['_v'] === value;
  } catch (e) {
    return false;
  }
}
 ...
 */
function _writeScalar(b, keyIndent, key, value) {
...
function _sortedStringKeys(m) {
  return Object.keys(m)
    .map((k) => String(k))
    .sort();
}

function _writeHeader(b, modelVersion) {
 ...
 * has.
 */
function decode(yamlText) {
  const root = yamlText.trim() === '' ? null : yaml.parse(yamlText);
  const isMap = root !== null && typeof root === 'object' && !Array.isArray(root);
