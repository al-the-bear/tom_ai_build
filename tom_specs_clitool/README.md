...
| `check_section_citations.dart` | Resolve every `§` citation in `tom_specs_model/doc` against `index.md`'s citation convention — a bare `§N` means *this* document. Exits `1` on a citation that resolves to no heading. The gate is **closed**: its default corpus is the doc folder, every TomSpecs README (`defaultCitedReadmes`), every package `doc/` folder (`defaultCitedDocFolders`) and the Dart doc comments of every citing source tree (`defaultCitedSourceRoots`). The last two enumerate *roots* and discover the files beneath them, so a guide or a source file added tomorrow is gated the day it is written. `--extra <file>` adds one more file to that corpus, `--doc-folder <dir>` adds a package `doc/` tree, `--doc` retargets the doc folder and `--source` the source roots, and `--no-default-readmes` / `--no-default-doc-folders` / `--no-default-sources` drop any default set; `--verbose` lists every resolution. | `--doc` `--extra` `--default-readmes` `--doc-folder` `--default-doc-folders` `--source` `--default-sources` `--verbose` |
