...
| Document | Authority for |
|----------|---------------|
| `codespecs_mapping.md` | The **grounding** document — *which SOM section feeds which part*: the four pillars and `tom_core`-family basis (§1.1/§1.1b), the neutral vocabulary (§1.2), the parts catalogue and three-project output (§4), the per-part code-basis gap analysis and attribute surfaces (§5), the server contract (§7), the SOM derivation with the section-coverage matrix, the closed-choice design and the CodeSpecs-versus-follow-up split and its taxonomy (§8), the bidirectional DocSpecs↔CodeSpecs link (§9), and the open-work index (§10) |
| `codespecs_derivation_contract.md` | The **derivation contract** — *what code comes out*: one entry per active `Cs*` marker giving its input SOM fields, the exact Dart emitted and its `tom_core`-family superclass, per-argument derivation, locus, cross-references and back-links (§3), plus the universal naming rules N1–N9 and stub-body rules (§2), a worked end-to-end example (§4), the annotation constructor shapes (§5) and the validator checks (§6). Where the two appear to disagree about emitted code, the derivation contract wins |
