...
| **2** | Shared contract | — (CE-API, CE-VA and CE-ID are READY; the CE-NT declarations and the CE-RP result envelope emit against their shipped `tom_core_codespecs` gap classes). Two cross-part edges emit as **untyped strings**, so the compile-time integrity check §5.23 exists to provide cannot run over them: a CE-NT channel's fallback, whose referent is undecided, and the CE-RP shared shapes' share of the four CE-RP targets | **V** | `csrb16`, `csrc3` |
...
**Three verification blockers stand besides, all of mode V.** Each is a cross-part
edge held as an id string where §5.23 would give a typed const, so the compiler —
the designated cross-part integrity checker — cannot see whether the reference
resolves: `TomJobDeclaration.targetRefs` at slice 7 (`csrc1`), CE-RP's four
targets at slices 2 and 3 (`csrc3`), and a CE-NT channel's fallback at slice 2,
where the referent itself is still undecided (`csrb16`). None blocks emission;
each removes a guarantee the slice order otherwise gives for free.
