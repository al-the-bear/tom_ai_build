 ...
 * `tom_som_javascript_runtime/tests/spec_meta_test.js` (itself a port of
 ...
 *
 * Build with `tsc`, then run `node dist/tests/spec_meta_test.js`. Exit code
 * 0 == all green.
 */
...
function _raises(fn: () => unknown): boolean {
  try {
    fn();
    return false;
  } catch (e) {
...
function buildFixtureTree(): SomMetaTree {
  ...
  const risk = (member: string): SomMetaNode =>
    new SomMetaNode({
      className: 'Risk',
      memberName: member,
      sectionId: 'RISK',
...
function testMetadataSlots(): void {
  const tree = buildFixtureTree();
...
function testItemPath(): void {
  ...
  const nested = tree
    .byId('GOAL-ITEM-LST')!
    .elementNode!.childByMember('subTasks')!;
