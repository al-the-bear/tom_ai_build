...
/**
 * DocSpecs-conform Markdown codec for a TomSpecs document (SOM §11) — a
 ...
 * `*.docspecs.yaml` format, SOM §12) so the generated schema's `pattern-check-id`
 ...
 * DocSpecs Markdown import/export format (SOM §11).
 */
class SpecDocumentMarkdown {
   ...
   * document. Throws an {@link Error} on the two conditions this format cannot
  ...
  _writeListItems(b, node, listPath, depth) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const pos = i + 1;
      // YRD3: a stored `@SectionId` (AA1 generated or a
   ...
   * (SOM §12.8).
   */
  _undeclaredFormFields(node, path) {
  ...
  _checkFormDeclared(node, path) {
    const undeclared = this._undeclaredFormFields(node, path);
    if (undeclared.length === 0) {
      return;
    }
    throw new Error(
      `form at "${path}" holds a field "${undeclared[0]}" unknown to the ` +
   ...
   * so the section is still *visited* and {@link _writeForm} refuses. Without
   * it a form holding nothing but undeclared fields would vanish unseen, which
   * is the silent drop this check exists to prevent.
   */
  _formHasValues(node, path) {
    ...
    return this._undeclaredFormFields(node, path).length > 0;
  }

  /**
   * Writes a `@Form` section body: the preamble content (when set) followed by
   * one `FieldName: value` group per populated field (SOM §11.4). Throws when
   * the form holds a field the model does not declare.
   */
  _writeForm(b, node, path) {
    this._checkFormDeclared(node, path);
    const fields = node.form !== null && node.form !== undefined ? node.form.fields : [];
  ...
  static _writeHeading(b, depth, id, title, codeSpec) {
    ...
    const meta =
      codeSpec !== null && codeSpec !== undefined && codeSpec !== ''
        ? ` codeSpec="${codeSpec}"`
        : '';
    b.writeln(`${'#'.repeat(depth)} <!--[${id}]${meta}--> ${title}`);
    b.writeln();
  }
  ...
  _prepareValue(value, path) {
    ...
    for (const line of collapsed.split('\n')) {
      if (fence.inFence) {
        out.push(line); // SOM §11.3: fences shield their lines.
...
SpecDocumentMarkdown._codeSpecPattern = /codeSpec=(?:"([^"]*)"|'([^']*)'|([^,\s>]+))/;
...
class _Parser {
   ...
   */
  _openItemHeading(level, container, listNode, id, title, codeSpec, lineNo) {
    ...
    if (anon !== null) {
      this._openItem(
        level, listPath, listNode, parseInt(anon[1], 10), null, title, codeSpec, lineNo,
      );
      return;
    }
    ...
    if (pattern !== null) {
      ...
      if (_Parser._patternMatches(pattern, id)) {
        this._openItem(level, listPath, listNode, null, id, title, codeSpec, lineNo);
        return;
      }
    }
   ...
   */
  _openItem(level, listPath, listNode, n, storedId, title, codeSpec, lineNo) {
