...
| CE-DS | DeviceSettings | `deviceSettings` | `@CsDeviceSetting` | the `tom_core` property/settings classes (§5.16, §11); the (user, device)-scoped store under them is a runtime gap (§4.1.1, `tcca11`) |
...
| CE-JB | BackgroundJob | `backgroundJob` | `@CsJob` | `TomJobBase` over the `TomCommand` / `TomExecutor` / `TomWorker` isolate-pooling substrate, `TomJobDefinition` / `TomSchedule` / `TomScheduler` / `TomJobStore` / `TomLeaseLock` / `TomJobDispatcher` (`tom_core_kernel`, `tombase/scheduling/`) — **the whole runtime half is reused**; `TomJobDeclaration` (`tom_core_codespecs`) → **narrow gap** for the deployment-and-ownership envelope only. The work body is compilable **pseudo-code** over a later-injected **abstract service class** (§3, §5.29) |
...
- **READY TO EMIT, BLOCKED AT RUNTIME — the class to build on exists, a
  capability under it does not:** CE-EL and CE-FM (unguarded text-controller
  write, `tcca15`), CE-DB (per-isolate static transaction scope, `tcca14`),
  CE-AU (challenge and verification only — no enrolment, no method choice on the
  wire, `tcca3`/`tcca4`), CE-DS (no (user, device)-scoped store, `tcca11`),
  CE-UP (server side of the round trip unwired, `tcca10`). Each emits and
  compiles; each yields an application that does not yet run. CE-EL carries the
  one **E(lossy)** gap besides — a *Choice* value's per-value copy (`tcca9`).
...
- **READY VIA `tom_core_codespecs` — the part reuses `tom_core` plus one
  ...
  substrate. **CE-UP appears here and in the runtime-blocked class both**: its
  gap *class* exists, and the server-side wiring under it does not.
...
**4 — The one gap the survey found, and its size.** The only workflow primitive
the substrate lacked was a **one-shot absolute deadline** ("fire once at a given
instant, then never") — the timer wait. It was never an architectural hole:
`TomSchedule` is a pure `DateTime? nextFireAfter(DateTime from)` in which `null`
already means *"never again"*, so a one-shot schedule is a small subclass
**inside the existing contract**. That the missing piece was this small is itself
an argument against the engine — the capability a workflow runtime is usually
reached for was, here, one class. It ships as `TomOnceSchedule` in CE-JB's
schedule family, alongside cron, calendar, interval and event.
...
Availability was never the whole criterion. Once `csex9` and `csex10` closed the
substrate gaps under CE-LG and CE-NT, and §4.3.1 surveyed CE-WF, all three stood
decidable at the same moment — and were decided against **one** test rather than
...
skeleton, and the one exception is lossy rather than blocking — a
...
The remaining SOM-side work is unchanged: the sections that do not yet carry the
surface a marker's arguments consume (`csrb13`), plus `csrc1` at slice 7. One
emission question is open on the code side and depends on no `tom_core` change —
an optional SOM field has no stated emission, because neither this document nor
the derivation contract names the `TomN*` nullable observable family that
`tom_core_kernel` ships (`qrc1`). The implied sequence: the four `tom_core`
todos and the nullable-emission decision run in parallel with the SOM gaps, then
the standing ownership question (`csrb1`).
...
| CE-DS | `TomProperty<T>` family + settings holders (`tom_core`) | Model **device settings** — user-specific settings of a user-owned device, persisted on the device and keyed by the signed-in user (§11). The marker and the declaration shape are settled; the holder that persists a value against a (user, device) key is a runtime gap (§4.1.1, `tcca11`). |
...
| CE-JB | `TomCommand` / `TomExecutor` / `TomWorker` isolate-pooling substrate (`tom_core_kernel`) — the work-body engine that runs a unit of work off the request thread | **Job definitions over the operational model**: `@CsJob` names a trigger (`cron \| calendar \| event`) + a work definition + target refs (CE-DB entities / CE-RP reports the job acts on) + retry/backoff/timeout/failure-alerting, distinct from request-driven CE-API. The **work body is compilable pseudo-code** (§3) calling a later-injected **abstract service class** whose methods carry detailed doc-comments; server project. The job base class (`TomJobBase`), scheduler runtime, job queue, multi-node locking and the pluggable-execution seam (`TomJobDispatcher`) are all reused from `tom_core_kernel`'s `scheduling` module; `tom_core_codespecs` adds only the deployment/ownership envelope `TomJobDeclaration`. `tom_process_monitor` is reference only, not a `tom_core`-family class (§5.29). |
...
(`tcca14`), not a missing marker. Until it closes, the *unit of work* an
access-object declares is honoured only under a single in-flight request.
...
- **Substrate status of the two user-scoped resolvers.** The precedence lattice
  ...
  parameter, by design (`tcca11`). **CE-UP's server side is unwired** —
  `TomUserSettingsStore` has one in-memory implementation and no caller outside
  `tom_core_codespecs`, and no `tom_core_server` code handles
  `TomGetSettingsMessage` (`tcca10`). Both are runtime gaps: the declarations
  emit and compile, and the persisted arm of each precedence chain resolves to
...
`@CsIdentityAttribute(placement: IdentityAttributePlacement.public)` — or
...
(`tcca3`, `tcca4`): a `@CsAuth` CodeSpec emits and compiles either way, and a
deployment that declares enrolment or a choice of methods gets neither.
...
| 3 Target references | `TomJobDeclaration.targetRefs` | `tom_core_codespecs` | On the declaration's constructor. **This is the one residual gap**: the member is `List<String>`, whereas this section and §4.4.1 require `Type` literals for CE-DB targets and `CsReportRef` for CE-RP targets. Tracked as `csrc1`. |
...
A todo that names a `tcca*` id waits on the `tom_core` quest
(`_ai/quests/tom_core/todos.tom_core.todo.yaml`) and cannot be executed before
that id lands; its work against *this* document is to state the position
afterwards.
