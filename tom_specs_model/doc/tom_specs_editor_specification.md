...
| V2 | Add **one new top-level root class that points to all 13 document roots**, so the entire spec is a single object model/tree with one true root (resolves the root-local-data concern: everything hangs off the one tree). |
...
| S1 | **One global schema plus 12 derived.** |
...
| N9 | The new top-level class is the **canonical tree root**, **not a document node of its own** (no `@SectionId`, not a 14th sibling). Serialization must emit the document **once** — the projection roots must **not** produce duplicate copies of the shared SBP content (§14, §15.1). |
...
| N12 | The connection is the existing **`@MapsTo` / `@DetailedIn`** links. A **null section is null in the SBP target too** (one shared tree — no divergence). This is only ever a question at the root level; the underlying **invariant** is that **a projection root contains no content that is not also in the Solution Blueprint** — this **must be validated** (added to the §8.6 validator). |
...
- `tom_specs_core` annotations; `tom_specs_model` 13 roots. **Crucial semantics:** the 12 Phase 3 roots are `@Document(basedOn:[SolutionBlueprint])` and aggregate **the same** SBP classes via typed fields with `@MapsTo`/`@DetailedIn`. So a spec's single source of truth is the **SolutionBlueprint instance**; the other roots are **projections** over it (this underpins Q9 — see §14).
...
- **`tom_som_dart_v0`** (pkg `tom_som_dart_v0`, `publish_to: none`, generated `lib/tom_som_dart_v0.dart`) is the **typed facade for model major 0**: one class per SOM class (~3080 classes), each `extends SomNode`, mirroring the 13 roots (`D00SolutionBlueprint` … `D12TransitionRolloutPlan`). Typed access reads/writes through the runtime, so it stays a thin, validated projection of the underlying document tree rather than a second copy of state. See `tom_som_dart_v0/example/a_typed_access.dart` (`D00SolutionBlueprint(doc)` over a live document).
...
Each Forge **application** (DocSpecs / CodeSpecs / Implementation) owns an **AgentContext = `{guidelines_specification.md`, tool set, D4rt scope profile`}`**. **Selecting the application** swaps all three: which `guidelines_specification.md` briefs the agent (role + scripting guide, [`d4rt_and_llm_tools.md`](d4rt_and_llm_tools.md) §11), which §8 tools are registered, and which D4rt scripting **scopes** (`spec`/`files`/`memory`) are available. The DocSpecs app enables all three scopes; the other apps define their own profiles later. The scripting **engine plane** (D4rt host + scope registry + file facade + search index + RAG memory) is detailed in the linked spec; see §21 for the placement/unblock note.
...
**Author-time only (MCP tools, not in-script):** grep-like **search** (`model_search` over the Layer-A object model) and **reflection** (`model_get_section` / `model_children`). The draft's in-script `doc_search`/`doc_search_iterate`/`doc_reflect`/`doc_add_node` MCP tools and `script_author`/`validate`/`run` MCP tools, the in-script `tom_som` reflection/typed-facade layer, and `mem_refresh` are **deferred** (see `guidelines_specification.md` §10). A script discovers structure by attempting `spec.addChild` (validates + throws), walking `spec.listItems`, and `memory.recall`; scripts run through the editor's in-process `SpecScriptRunner` (`AgentToolsModule.scriptRunner`).
...
- A spec is **one document** rooted in **one new top-level class** (added to `tom_specs_model`) that **references all 13 document roots** (V2). This gives the editor a single object model/tree with one true root; the SolutionBlueprint instance remains the source of truth for shared content, and the other roots hang off the same tree under that single root.
- **The container is the canonical tree root, not a document node (N9):** it carries **no `@SectionId`**, is **not** rendered as a 14th sibling document, and never appears in the navigator as content. It exists purely to give load/save/undo/snapshot a single object to operate on, **owns the `toYaml` global save** (SBP-only, §15.1), and runs the **connect pass** that binds projection roots to live SBP sections before any individual-root write (N11). Tooling (`ModelJsonExporter`, validator, outliner) must treat it as the canonical root, not as another document (Q-N11).
...
- Editing through any projection edits the shared underlying sections; the compact tree, change log, and review state are consistent across projections (keyed by section id / structural path).
...
- **Single true root (V2):** rather than leaving the 13 roots as 13 disconnected entry points, `tom_specs_model` gains a **new top-level container class** whose fields are the 13 document roots. The editor loads/serializes/snapshots **that one object**, so:
  - the whole spec is genuinely one tree (no special-casing of "which root am I in" for save/undo/snapshot);
  - any **root-local** data a Phase 3 root might carry (beyond pure SBP projections) now has a guaranteed home on the single tree — the earlier root-local-data concern is dissolved;
...
- **Cardinality is a per-element content constraint, not a required-section contract.** `min`/`max-count-in-document` bounds present list items; it is validated over depth-1 sections, so it is exact for a top-level list slot and a whole-document over-approximation when the same element type recurs under multiple parents (the semantically precise `subsection-types` min/max home awaits the connect-pass parent→child linkage). Because projection roots are **views, not authoring contracts (N12)**, every generated document slot is `optional: true` — no document-level required-section / `min-count` slot is emitted; the only required cardinality that ships is the per-element `@Min`. `pattern-check-text` is unemitted (no text-constraint annotation exists to map from).
...
- **Schema set (S1):** **one global schema** (Solution Blueprint) **plus 12 derived** (one per Phase 3 projection root) — 13 schemas total, matching the 13 roots.
...
4. **Add the canonical container root to `tom_specs_model` (V2, N9).** New top-level class referencing all 13 document roots; **no `@SectionId`**, not a document node. **Done:** class compiles; the existing model test suite still passes.
...
23. **DocSpecs schema generator (§16, Q8, S1/S2).** Generate 1 global + 12 derived `*.docspecs-schema.yaml` from the model annotations; bundle in-app + write to `.tom/docspecs-schema/`; version counts up from the model stamp. **Done:** generated schemas validate a known-good export.
...
Full design and settled decisions in [`d4rt_and_llm_tools.md`](d4rt_and_llm_tools.md) (§14). Summary of how it lands in this editor:

- **Scopes (§8):** the in-process `tom_spec_engine` exposes three D4rt **scopes**, each one global under one import — `spec` (eight-method `SpecApi` bound to the live controller), `files` (read-anywhere / write-`agent/scratchpad` facade), and `memory` (read-only `recall`/`recallPaths`). All mutating paths route through the **one document controller** → the single change log (§9). Author-time **search**/**reflection** stay in the `model_search`/`model_get_section`/`model_children` MCP tools; the draft's in-script `doc_search`/`doc_reflect`/`doc_add_node` and `script_*` MCP tools are deferred.
...
- **AgentContext (§7.3):** the `spec`/`files`/`memory` scopes coexist; the active application selects the scope profile + tool set + `guidelines_specification.md`. Per-application/phase isolation rides Tom Brain's **profiles + named sessions + named memory**.
...
- **Engine placement (resolved):** the former analyzer conflict (`tom_d4rt ^8` vs `tom_dart_editor ^10`) is **gone** — `tom_d4rt` is on `analyzer ^10`, so the pure-Dart `tom_spec_engine` links **in-process**; D4rt and the §8 tools call `SpecDocumentController` directly (no companion process, no reverse-RPC hop). This also unblocks live d4rt-flutter rendering (§13). The earlier "pure-Dart companion engine outside the Flutter closure" fallback is obsolete.
