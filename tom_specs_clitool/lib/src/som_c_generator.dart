...
/// `som_go_generator.dart` / `som_rust_generator.dart` (som_multiplatform_spec_model.md §10). The
...
/// archives the generated translation unit into a static (`build/libtom_som_c_v0.a`)
/// and a shared (`build/libtom_som_c_v0.so`) library. C has no package registry,
/// so the `Makefile` also emits a `pkg-config` file (`tom_som_c_v0.pc`, `Version`
/// = the model version, `Requires: tom_som_c_runtime`) and provides `make install`
/// (headers + libraries + `.pc`) and `make dist` (versioned source tarball)
/// targets, mirroring the hand-authored runtime. The emitted C source carries no
...
String _makefile(String runtimeRel, String version) {
  ...
  // `make RUNTIME_DIR=/abs/path`). C has no registry, so packaging is a
  // library + headers + pkg-config file (installed or vendored) plus a source
  ...
  return '# GENERATED by tom_specs_clitool SomCGenerator — do not edit by hand.\n'
      ...
      '\$(SHARED): \$(PICOBJ)\n'
      '\t\$(CC) -shared \$(SONAME_FLAG) \$(UNDEF_FLAG) -o \$@ \$^\n'
      '\n'
