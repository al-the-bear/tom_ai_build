...
same plane is
[`tom_som_cpp_runtime/doc/generic_access.md`](../../tom_som_cpp_runtime/doc/generic_access.md).
...
`tom_som_cpp_v0` is the **typed** access path: one generated type per document
...
`tom_som_cpp_runtime` actually holds — the runtime carries the sparse,
...
```bash
make install (then compile against pkg-config `tom_som_cpp_v0`)
```

Both halves are versioned to the TomSpecs **model version** and must move
together — `tom_som_cpp_v0` and `tom_som_cpp_runtime` always carry the
...
int main() {
  // 1 — open a document and wrap it in a typed root. RAII: the document is a
  // value that must outlive every facade bound to it.
  ...
  // 3 — read it back. Getters return std::string by value; nothing to free.
  std::cout << bp.content() << "\n";

  // 4 — validate against the model.
  std::ifstream in("meta/spec_model.meta.json");
  std::stringstream ss; ss << in.rdbuf();
  std::string err;
  std::unique_ptr<som::SpecModel> model =
      som::SpecModel::fromJsonStr(ss.str(), &err);
  std::cout << som::validateDocument(*model, doc).empty() << "\n";
  ...
  som::decodeYaml(*yaml, tom_som_v0_meta::d00SolutionBlueprintMetaTree(),
                  &decoded, &err);
  std::cout << decoded.document.content("SBP/content") << "\n";
  std::cout << decoded.modelVersion << "\n";
  return 0;
}
```
...
```bash
make && g++ -std=c++17 -Iinclude -I../tom_som_cpp_runtime/include \
      tutorial.cpp build/libtom_som_cpp_v0.a \
      ../tom_som_cpp_runtime/build/libtom_som_cpp_runtime.a -o tutorial && ./tutorial
```
...
./tool/regenerate_api_references.sh cpp_v0
```
...
- **Prefer the typed path.** Reach for the generic store only when the path is
  ...
  [`generic_access.md`](../../tom_som_cpp_runtime/doc/generic_access.md).
