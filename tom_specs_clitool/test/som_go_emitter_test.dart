...
void _expectGoBuilds(String source, String metaSource) {
  ...
  try {
    ...
    File(
      p.join(dir.path, 'tom_som_go_v0_meta.go'),
...
void main() {
  ...
  group('SomGoEmitter', () {
    ...
    test(
      'the generated module go-builds and go-vets clean against the runtime',
      () {
        _expectGoBuilds(
          SomGoEmitter(_fixtureModel()).generateLibrary(),
    ...
    test('the emitted module is gofmt-stable by construction', () {
      ...
      final goroot = Process.runSync(go, [
        'env',
        'GOROOT',
    ...
    test('a New-prefixed type name does not collide with a constructor', () {
      final source = SomGoEmitter(
        SpecModel.fromJson(_newPrefixCollisionJson()),
      ).generateLibrary();
    ...
    test(
      ...
      () {
        ...
        expect(
          SomGoEmitter(stamped, versionLabel: 'v0').modelVersionString,
          '1.3',
        );
        expect(
          SomGoEmitter(stamped, versionLabel: 'v9').modelVersionString,
          '1.3',
        );
      },
    );
    ...
    test('accessors colliding under Pascal-casing are de-duplicated', () {
      final source = SomGoEmitter(
        SpecModel.fromJson(_accessorCollisionJson()),
      ).generateLibrary();
    ...
    test('a pattern-bearing list emits its @SectionIdPattern; a scalar list '
      ...
      expect(
        RegExp(
          r'func \(x \*SolutionBlueprint\) Risks\(\)[\s\S]*?'
    ...
    test('no flat path-constant holder is emitted; the meta trees are '
      ...
      expect(source, contains('som.FromYaml(yaml, SolutionBlueprintMetaTree)'));
      expect(source, contains('som.FromFile(path, SolutionBlueprintMetaTree)'));
    });
  });
}
